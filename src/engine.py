...
_INDEX_PATH_RE = re.compile(
    ...
    r"|/(?:internships|university[-_]?recruiting|campus[-_]?recruiting|early[-_]?careers?|student[-_]?programs?|graduate[-_]?programs?|university[-_]?programs?|job[-_]?search|life[-_]?at(?:[-_][^/]+)?|team|meet[-_]?(?:the|our)[-_]?team|our[-_]?(?:team|people)|benefits|our[-_]?benefits|culture|our[-_]?culture|leadership|our[-_]?leadership|about[-_]?us|about|our[-_]?values|values|our[-_]?mission|locations|our[-_]?locations|diversity|inclusion|dei|our[-_]?dei|diversity[-_]?equity(?:[-_]?and)?[-_]?inclusion|our[-_]?story|faqs?|news|press|blog|our[-_]?blog|newsroom|press[-_]?releases?|our[-_]?news|investors?|investor[-_]?relations|sustainability|our[-_]?sustainability)/?$"
    r"|/(?:salaries|salary)(?:/|$)"
    r"|/apply/?$",
    re.I,
)
...
_GONE_LISTING_RE = re.compile(
    ...
    r"|(?:the|this)\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening)"
    r"\s+was\s+(?:withdrawn|cancelled|canceled|removed|taken\s+down|taken\s+offline|deactivated)"
    ...
    r"|we(?:'ve|\s+have)\s+(?:filled|closed|withdrawn|cancelled|canceled|removed|unposted)\s+this\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening)"
    r"|(?:the|this)\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening)"
    ...
    r"|(?:the\s+)?application\s+(?:window|period)\s+(?:has\s+closed|is\s+closed)"
...
_NON_SALARY_MONEY_RE = re.compile(
    ...
    r"\s+(?:in\s+)?(?:equity|stock(?:\s+options?)?|RSUs?|ESPP|employee\s+stock\s+purchase(?:\s+plan)?|severance)\b"
    r"|"
    r"\b(?:equity|stock(?:\s+options?)?|RSUs?|ESPP|employee\s+stock\s+purchase(?:\s+plan)?|severance)\s*(?:of|:)?\s*"
    r"(?:USD|US\$|\$)\s*[\d,]+(?:\.\d+)?(?:\s*k)?"
    r"(?:\s*(?:[-–—]|to)\s*(?:USD|US\$|\$)?\s*[\d,]+(?:\.\d+)?(?:\s*k)?)?"
    r"|"
    r"(?:USD|US\$|\$)\s*[\d,]+(?:\.\d+)?(?:\s*k)?"
    r"(?:\s*(?:[-–—]|to)\s*(?:USD|US\$|\$)?\s*[\d,]+(?:\.\d+)?(?:\s*k)?)?"
    r"\s+(?:(?:employee\s+)?(?:signing|sign-on|sign on|relocation|annual|target|performance|retention|referral|spot|cash|year-end|year end|holiday|discretionary|quarterly|incentive|sales|monthly|stay|completion|anniversary)\s+bonus|(?:signing|sign-on|sign on)\b)"
    r"|"
    r"\b(?:(?:employee\s+)?(?:signing|sign-on|sign on|relocation|annual|target|performance|retention|referral|spot|cash|year-end|year end|holiday|discretionary|quarterly|incentive|sales|monthly|stay|completion|anniversary)\s+bonus|(?:signing|sign-on|sign on))\s*(?:of|:)?\s*"
    r"(?:USD|US\$|\$)\s*[\d,]+(?:\.\d+)?(?:\s*k)?"
    r"(?:\s*(?:[-–—]|to)\s*(?:USD|US\$|\$)?\s*[\d,]+(?:\.\d+)?(?:\s*k)?)?"
    r"|"
    r"(?:USD|US\$|\$)\s*[\d,]+(?:\.\d+)?(?:\s*k)?"
    ...
    r"(?:USD|US\$|\$)\s*[\d,]+(?:\.\d+)?(?:\s*k)?"
    r"(?:\s*(?:[-–—]|to)\s*(?:USD|US\$|\$)?\s*[\d,]+(?:\.\d+)?(?:\s*k)?)?"
    r"\s+(?:PTO|vacation)\s+(?:buyback|cash[- ]?out|payout)\b"
    r"|"
    r"\b(?:PTO|vacation)\s+(?:buyback|cash[- ]?out|payout)\s*(?:of|:)?\s*"
    r"(?:USD|US\$|\$)\s*[\d,]+(?:\.\d+)?(?:\s*k)?"
    r"(?:\s*(?:[-–—]|to)\s*(?:USD|US\$|\$)?\s*[\d,]+(?:\.\d+)?(?:\s*k)?)?"
    r"|"
    r"(?:USD|US\$|\$)\s*[\d,]+(?:\.\d+)?(?:\s*k)?"
    ...
    r"(?:\s+(?:housing|living|relocation|meal|food|travel|wellness|phone|cell(?:phone)?|mobile|internet|commuter|parking|home\s+office|gym(?:\s+membership)?|fitness|child(?:[-\s])?care|dependent\s+care|health(?:care)?|WFH|work\s+from\s+home|remote\s+work|equipment|laptop|volunteer|tools))*"
    r"\s+stipend\b"
    r"|"
    ...
    r"(?:\s*(?:[-–—]|to)\s*(?:USD|US\$|\$)?\s*[\d,]+(?:\.\d+)?(?:\s*k)?)?"
    r"(?:\s*(?:/\s*mo(?:nth)?s?|(?:per|a)\s+mo(?:nth)?s?|monthly))?"
    r"(?:\s+(?:housing|living|relocation|car|vehicle|auto|phone|cell|internet|meal|food|travel|commuter|parking|home\s+office|gym(?:\s+membership)?|fitness|child(?:[-\s])?care|dependent\s+care|health(?:care)?|clothing|uniform))*"
    r"\s+allowance\b"
    r"|"
    r"\b(?:(?:monthly|housing|living|relocation|car|vehicle|auto|phone|cell|internet|meal|food|travel|commuter|parking|clothing|uniform)\s+)*allowance\s*(?:of|:)?\s*"
    r"(?:USD|US\$|\$)\s*[\d,]+(?:\.\d+)?(?:\s*k)?"
    r"(?:\s*(?:[-–—]|to)\s*(?:USD|US\$|\$)?\s*[\d,]+(?:\.\d+)?(?:\s*k)?)?"
    r"|"
    r"(?:USD|US\$|\$)\s*[\d,]+(?:\.\d+)?(?:\s*k)?"
