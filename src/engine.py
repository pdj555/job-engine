...
_ONSITE_WORKPLACE_RE = re.compile(
    ...
    r"(?:\s+(?!two\b|three\b|four\b|five\b|\d+\b)\S+){0,4}"
    r"\s+(?:two|three|four|five|\d+(?:\s*[-–]\s*\d+)?)\s+days?\b"
    r"|(?<!not )(?:required|expected|expect|need(?:s|ed)?|asked|requested|requests?|invited|encouraged|preferred|prefer|supposed|ought)\s+to\s+be\s+in\b"
    r"(?!\s+(?:the\s+)?(?:us|usa|united\s+states|uk|united\s+kingdom|europe|eu|emea|apac|anywhere|worldwide|globally|remote|meetings?|standups?|calls?|cloud|home)\b)"
    r"(?:\s+(?!two\b|three\b|four\b|five\b|\d+\b)\S+){0,4}"
    r"\s+(?:two|three|four|five|\d+(?:\s*[-–]\s*\d+)?)\s+days?\b"
    r"|(?<!not )(?:required|expected|expect|need(?:s|ed)?|asked|requested|requests?|invited|encouraged|preferred|prefer|supposed|ought)\s+to\s+work\s+(?:from|in)\b"
    ...
    r"|(?:must|should(?!n't)(?!\s+not))\s+work\s+(?:from|in)\b"
    r"(?!\s+(?:the\s+)?(?:us|usa|united\s+states|uk|united\s+kingdom|europe|eu|emea|apac|anywhere|worldwide|globally|remote|home|meetings?|standups?|calls?|cloud)\b)"
    r"(?:\s+(?!two\b|three\b|four\b|five\b|\d+\b)\S+){0,4}"
    r"\s+(?:two|three|four|five|\d+(?:\s*[-–]\s*\d+)?)\s+days?\b"
