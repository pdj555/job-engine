...
_GONE_LISTING_RE = re.compile(
    ...
    r"\s+could\s+not\s+be\s+found\b"
...
_NON_SALARY_MONEY_RE = re.compile(
    ...
    r"(?:USD|US\$|\$)\s*[\d,]+(?:\.\d+)?(?:\s*k)?"
    r"(?:\s*(?:[-–—]|to)\s*(?:USD|US\$|\$)?\s*[\d,]+(?:\.\d+)?(?:\s*k)?)?"
    r"\s+(?:HSA|FSA|HRA|health\s+savings\s+account|health\s+reimbursement\s+arrangement)\s+(?:contribution|benefit)\b"
    r"|"
    r"\b(?:HSA|FSA|HRA|health\s+savings\s+account|health\s+reimbursement\s+arrangement)\s+(?:contribution|benefit)\s*(?:of|:)?\s*"
    r"(?:USD|US\$|\$)\s*[\d,]+(?:\.\d+)?(?:\s*k)?"
    r"(?:\s*(?:[-–—]|to)\s*(?:USD|US\$|\$)?\s*[\d,]+(?:\.\d+)?(?:\s*k)?)?"
    r"|"
    r"(?:USD|US\$|\$)\s*[\d,]+(?:\.\d+)?(?:\s*k)?"
