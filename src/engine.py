...
_NON_SALARY_MONEY_RE = re.compile(
    ...
    r"(?:" + _ANY_PERIOD + r")?"
    r"\s+(?:in\s+)?(?:equity|stock(?:\s+options?)?+|RSUs?|ESPP|employee\s+stock(?:\s+purchase(?:\s+plan)?)?+|restricted\s+stock|option\s+grant|severance)\b(?!\s*(?:of|:)?\s*(?:USD|US\$|\$)\s*[\d,])"
    r"|"
    ...
    r"(?:\s*(?:[-–—]|to)\s*(?:USD|US\$|\$)?\s*[\d,]+(?:\s+\d{3})*(?:\.\d+)?(?:\s*k)?)?"
    r"(?:" + _ANY_PERIOD + r")?"
    r"\s+(?:HSA|FSA|HRA|QSEHRA|ICHRA|pension|529|401(?:k|\(k\))|health\s+savings\s+account|health\s+reimbursement\s+arrangement)(?:\s+(?:contribution|benefit))?+\b(?!\s+in)(?!\s*(?:(?:match|matching|contribution|benefit)\s+)?(?:of|:)?\s*(?:USD|US\$|\$)\s*[\d,])"
    r"|"
    r"\b(?:HSA|FSA|HRA|QSEHRA|ICHRA|pension|529|401(?:k|\(k\))|health\s+savings\s+account|health\s+reimbursement\s+arrangement)(?:\s+(?:contribution|benefit))?\s*(?:of|:)?\s*"
    r"(?:USD|US\$|\$)\s*[\d,]+(?:\s+\d{3})*(?:\.\d+)?(?:\s*k)?"
    r"(?:\s*(?:[-–—]|to)\s*(?:USD|US\$|\$)?\s*[\d,]+(?:\s+\d{3})*(?:\.\d+)?(?:\s*k)?)?"
    r"|"
    r"(?:USD|US\$|\$)\s*[\d,]+(?:\s+\d{3})*(?:\.\d+)?(?:\s*k)?"
