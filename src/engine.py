...
_DAY_TAIL = (
    r"\s*(?:"
    r"\.?\s*(?:/\s*(?:d(?:ays?)?|diem)|p\s*/\s*(?:d(?:ays?)?|diem)|(?:p|per|an|a)\.\s*(?:days?|diem)|p\s+(?:days?|diem)|(?:per|a)\s*/?\s*(?:d(?:ays?)?|diem)|days?(?!\s+(?:one|of|in)\b))\b"
...
_HOURS_RE = re.compile(
    ...
    r"|(?:hours?|hrs?)\s*(?:(?:worked|working|scheduled)\s+)?(?:(?:p(?:er[\s.]*)?\.?|/|a|for(?:\s+the)?)\s*)?(?:wk|weeks?|weekly)\s*(?:scheduled\s*)?[:=\-–—]?\s*(\d{1,2}(?:\.\d+)?)\+?"
    ...
    r"(?!\s+(?:meeting|standup|stand-up|sync|call|all-?hands))"
    r"|(?:hours?|hrs?)\s*(?:worked\s*)?[:=\-–—]\s*(\d{1,2}(?:\.\d+)?)\+?\s*(?:p(?:er[\s.]*)?\.?|/|a\s+)(?:wk|weeks?|weekly)\b"
    r"|work[\s-]*weeks?\s*[:=\-–—]\s*(\d{1,2}(?:\.\d+)?)\+?\s*(?:hours?|hrs?)?\b"
    r"|(?:hours?|hrs?)\s+of\s+(?:the\s+)?(?:work(?:ing)?|scheduled)\s*(?:(?:p(?:er[\s.]*)?\.?|/|a)\s*)?(?:wk|weeks?|weekly)\s*[:=\-–—]?\s*(\d{1,2}(?:\.\d+)?)\+?"
    r"|(?<![\d.])(\d{1,2}(?:\.\d+)?)\+?[\s-]*hpw\b"
    r"|(?<![\d.])(\d{1,2}(?:\.\d+)?)\+?[\s-]*h(?:p)?\s*/\s*w\b"
    r"|(?<![\d.])(\d{1,2}(?:\.\d+)?)\+?[\s-]*(?:hours?|hrs?|h)\.?\s+p(?:er[\s.]*)?\.?\s*/?\s*w(?:(?:ee)?ks?)?\b"
