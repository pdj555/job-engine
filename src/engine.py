...
_GONE_LISTING_RE = re.compile(
    ...
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening|search)"
    r"\s+back\b"
    r"|(?<!once )(?<!after )(?<!when )(?:the|this)\s+search\s+(?:is|has\s+been|was|has)\s+(?:closed|filled|expired|lapsed|withdrawn|removed|taken\s+down|taken\s+offline|(?:un|de[-\s]?)published|(?:un|de[-\s]?)released|(?:un|de[-\s]?)syndicated|(?:un|de[-\s]?)advertised|archived|(?:un|de[-\s]?)activated|deleted|(?:un|de[-\s]?)posted|pulled|discontinued)\b"
    ...
    r"|we(?:'ve|\s+have)?\s+(?:already\s+)?(?:filled|closed|withdrawn|withdrew|cancelled|canceled|removed|(?:un|de[-\s]?)posted|(?:un|de[-\s]?)published|(?:un|de[-\s]?)released|(?:un|de[-\s]?)syndicated|(?:un|de[-\s]?)advertised|(?:un|de[-\s]?)activated|archived|deleted|discontinued|pulled|rescinded|abandoned|called\s+off|shelved|tabled|sunset|retired|dropped|released|frozen|killed|scrapped|postponed|suspended|deferred|parked|mothballed|hibernated|stalled|halted|idled|inactivated|iced|shuttered|sidelined|canned|axed|retracted|voided|revoked|recalled|boxed|folded|(?:un|de[-\s]?)listed|junked|ditched|dumped|tossed|chucked|scrubbed|spiked|bagged|binned|trashed|shredded|pulped|nuked|discarded|purged|yanked|wiped|expunged|jettisoned|redacted|annulled|nullified|vacated|invalidated|quashed|scuttled|torpedoed|obliterated|erased|vaporized|pulverized|pulverised|atomized|annihilated|extinguished|extirpated|excised|abrogated|repealed|overruled|overturned|dissolved|disbanded|liquidated|aborted|deep[-\s]sixed|crushed|flattened|buried|gutted|whacked|snuffed|sunsetted|decommissioned|dismantled|demolished|wrecked|de[-\s]?funded|eliminated|abolished|disestablished|un[-\s]?funded|superseded|reallocated|zero[-\s]?funded|backfilled|de[-\s]?staffed|un[-\s]?staffed|un[-\s]?resourced|de[-\s]?resourced|un[-\s]?allocated|de[-\s]?allocated|un[-\s]?budgeted|de[-\s]?budgeted)\s+this\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening|search)"
    ...
    r"|(?<!once )(?<!after )(?<!when )(?:the|this)\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening)"
    r"\s+is\s+(?:(?:un|de[-\s]?)published|(?:un|de[-\s]?)released|(?:un|de[-\s]?)syndicated|(?:un|de[-\s]?)advertised|(?:un|de[-\s]?)posted|archived|(?:un|de[-\s]?)activated)\b"
    r"|(?<!once )(?<!after )(?<!when )(?<!this )(?<!the )(?<!a )(?:sorry,\s+)?"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening)"
    r"\s+is\s+(?:(?:un|de[-\s]?)published|(?:un|de[-\s]?)released|(?:un|de[-\s]?)syndicated|(?:un|de[-\s]?)advertised|(?:un|de[-\s]?)posted|archived|(?:un|de[-\s]?)activated)\b"
