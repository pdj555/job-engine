...
_GONE_LISTING_RE = re.compile(
    ...
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening|search)"
    r"|we(?:'ve|\s+have)?\s+stopped\s+(?:accepting|taking|receiving|collecting|processing|reviewing)\s+(?:new\s+)?(?:applications|applicants)\b(?!\s+from)"
    ...
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening|search)"
    r"|no\s+longer\s+(?:accepting|taking|receiving|collecting|processing)\s+(?:new\s+)?(?:applications|applicants|candidates)\b(?!\s+from)"
