...
_NON_SALARY_MONEY_RE = re.compile(
    ...
    r"(?:\s*(?:[-–—]|to)\s*(?:USD|US\$|\$)?\s*[\d,]+(?:\s+\d{3})*(?:\.\d+)?(?:\s*k)?)?"
    r"(?:" + _ANY_PERIOD + r")?"
    r"\s+(?:(?:OTE|on[- ]target\s+earnings|commission|TC|total\s+comp(?:ensation)?|incentive\s+compensation|variable\s+(?:pay|compensation)|deferred\s+compensation|(?:long|short)[-\s]term\s+incentive|(?:target|sales|cash|annual|performance)\s+incentive)\b(?!\s*(?:(?:clawback|recoupment|recovery|repayment|holdback|forfeiture|forfeit|penalty|draw)\s+)?(?:of|:)?\s*(?:USD|US\$|\$)\s*[\d,]))"
    r"|"
