...
_NON_SALARY_MONEY_RE = re.compile(
    ...
    r"(?:\s*(?:[-–—]|to)\s*(?:USD|US\$|\$)?\s*[\d,]+(?:\s+\d{3})*(?:\.\d+)?(?:\s*k)?)?"
    r"(?:" + _ANY_PERIOD + r")?"
    r"\s+relocation(?:\s+(?:bonus|assistance|package|stipend|credit))?+\b(?!\s+to)(?!\s*(?:of|:)?\s*(?:USD|US\$|\$)\s*[\d,])"
    r"|"
    ...
    r"(?:\s*(?:[-–—]|to)\s*(?:USD|US\$|\$)?\s*[\d,]+(?:\s+\d{3})*(?:\.\d+)?(?:\s*k)?)?"
    r"(?:" + _ANY_PERIOD + r")?"
    r"\s+(?:HSA|FSA|HRA|QSEHRA|ICHRA|pension|529|401(?:k|\(k\))|health\s+savings\s+account|health\s+reimbursement\s+arrangement)(?:\s+(?:contribution|benefit|account|fund|credit))?+\b(?!\s+in)(?!\s*(?:(?:match|matching|contribution|benefits?|account|fund|credit)\s+)?(?:of|:)?\s*(?:USD|US\$|\$)\s*[\d,])"
    r"|"
    r"\b(?:HSA|FSA|HRA|QSEHRA|ICHRA|pension|529|401(?:k|\(k\))|health\s+savings\s+account|health\s+reimbursement\s+arrangement)(?:\s+(?:contribution|benefit|account|fund|credit))?\s*(?:of|:)?\s*"
    r"(?:USD|US\$|\$)\s*[\d,]+(?:\s+\d{3})*(?:\.\d+)?(?:\s*k)?"
    r"(?:\s*(?:[-–—]|to)\s*(?:USD|US\$|\$)?\s*[\d,]+(?:\s+\d{3})*(?:\.\d+)?(?:\s*k)?)?"
    r"|"
    r"(?:USD|US\$|\$)\s*[\d,]+(?:\s+\d{3})*(?:\.\d+)?(?:\s*k)?"
