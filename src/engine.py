...
_NON_SALARY_MONEY_RE = re.compile(
    ...
    r"(?:\s+(?:housing|living|relocation|meal|food|travel|wellness))?"
    r"\s+stipend\b"
    r"|"
...
_ONSITE_WORKPLACE_RE = re.compile(
    ...
    r"|work\s+from\s+(?:the\s+|our\s+|an\s+)?(?:office|campus|lab(?:oratory)?)\b"
    r"|\blab(?:oratory)?\s+(?:role|position|job)\b"
