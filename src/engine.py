...
_NON_SALARY_MONEY_RE = re.compile(
    ...
    r"(?:USD|US\$|\$)\s*[\d,]+(?:\.\d+)?(?:\s*k)?"
    r"(?:\s*(?:[-–—]|to)\s*(?:USD|US\$|\$)?\s*[\d,]+(?:\.\d+)?(?:\s*k)?)?"
    r"\s+(?:tuition(?:\s+(?:reimbursement|assistance|benefit))?(?!\s+in)|education\s+(?:reimbursement|assistance|benefit)|student[-\s]loans?(?:\s+repayment)?)\b"
    r"|"
    r"\b(?:tuition(?:\s+(?:reimbursement|assistance|benefit))?|education\s+(?:reimbursement|assistance|benefit)|student[-\s]loans?(?:\s+repayment)?)\s*(?:of|:)?\s*"
    r"(?:USD|US\$|\$)\s*[\d,]+(?:\.\d+)?(?:\s*k)?"
    r"(?:\s*(?:[-–—]|to)\s*(?:USD|US\$|\$)?\s*[\d,]+(?:\.\d+)?(?:\s*k)?)?"
    r"|"
    r"(?:USD|US\$|\$)\s*[\d,]+(?:\.\d+)?(?:\s*k)?"
