...
_NON_SALARY_MONEY_RE = re.compile(
    ...
    r"(?:\s*(?:[-–—]|to)\s*(?:USD|US\$|\$)?\s*[\d,]+(?:\s+\d{3})*(?:\.\d+)?(?:\s*k)?)?"
    r"(?:" + _ANY_PERIOD + r")?"
    r"\s+(?:overtime|on[-\s]call|shift\s+differential|double[-\s]?time|time[-\s]and[-\s](?:a|one)[-\s]half|comp(?:ensatory)?[-\s]?time)\b"
    r"|"
    r"\b(?:overtime(?:\s+paid)?|on[-\s]call(?:\s+pay)?|shift\s+differential|double[-\s]?time|time[-\s]and[-\s](?:a|one)[-\s]half|comp(?:ensatory)?[-\s]?time)\s*(?:at|of|:)?\s*"
    r"(?:USD|US\$|\$)\s*[\d,]+(?:\s+\d{3})*(?:\.\d+)?(?:\s*k)?"
    r"(?:\s*(?:[-–—]|to)\s*(?:USD|US\$|\$)?\s*[\d,]+(?:\s+\d{3})*(?:\.\d+)?(?:\s*k)?)?"
    r"(?:" + _HOUR_TAIL + r")?"
    r")"
