...
_NON_SALARY_MONEY_RE = re.compile(
    ...
    r"(?:\s*(?:[-–—]|to)\s*(?:USD|US\$|\$)?\s*[\d,]+(?:\s+\d{3})*(?:\.\d+)?(?:\s*k)?)?"
    r"(?:" + _ANY_PERIOD + r")?"
    r"\s+(?:overtime|on[-\s]call|shift\s+differential|double[-\s]?time|time[-\s]and[-\s](?:a|one)[-\s]half|comp(?:ensatory)?[-\s]?time|night[-\s]?differential|weekend[-\s]?differential|holiday[-\s]?(?:differential|pay|rate)|hazard[-\s]?pay|danger[-\s]?pay|hardship[-\s]?pay|call[-\s]?out(?:\s+pay)?|call[-\s]?back(?:\s+pay)?|standby(?:\s+pay)?|evening[-\s]?differential|graveyard[-\s]?differential|swing[-\s]?differential|(?:night|weekend)[-\s]?(?:premium|rate|pay)|(?:saturday|sunday)[-\s]?(?:premium|pay|rate)|call[-\s]?in(?:\s+pay)?|piece[-\s]?rate|show[-\s]?up(?:\s+pay)?|reporting[-\s]?(?:pay|time)|travel[-\s]?(?:pay|time)|meal[-\s]?penalty|split[-\s]?shift[-\s]?premium|portal[-\s]to[-\s]portal)\b"
    r"|"
    r"\b(?:overtime(?:\s+paid)?|on[-\s]call(?:\s+pay)?|shift\s+differential|double[-\s]?time|time[-\s]and[-\s](?:a|one)[-\s]half|comp(?:ensatory)?[-\s]?time|night[-\s]?differential|weekend[-\s]?differential|holiday[-\s]?(?:differential|pay|rate)|hazard[-\s]?pay|danger[-\s]?pay|hardship[-\s]?pay|call[-\s]?out(?:\s+pay)?|call[-\s]?back(?:\s+pay)?|standby(?:\s+pay)?|evening[-\s]?differential|graveyard[-\s]?differential|swing[-\s]?differential|(?:night|weekend)[-\s]?(?:premium|rate|pay)|(?:saturday|sunday)[-\s]?(?:premium|pay|rate)|call[-\s]?in(?:\s+pay)?|piece[-\s]?rate|show[-\s]?up(?:\s+pay)?|reporting[-\s]?(?:pay|time)|travel[-\s]?(?:pay|time)|meal[-\s]?penalty|split[-\s]?shift[-\s]?premium|portal[-\s]to[-\s]portal)\s*(?:at|of|:)?\s*"
    r"(?:USD|US\$|\$)\s*[\d,]+(?:\s+\d{3})*(?:\.\d+)?(?:\s*k)?"
    r"(?:\s*(?:[-–—]|to)\s*(?:USD|US\$|\$)?\s*[\d,]+(?:\s+\d{3})*(?:\.\d+)?(?:\s*k)?)?"
    r"(?:" + _HOUR_TAIL + r")?"
    r"|"
