...
_HOURS_RE = re.compile(
    ...
    r"(?!\s+(?:meeting|standup|stand-up|sync|call|all-?hands))"
    r"|(?:hours?|hrs?)\s*(?:(?:worked|(?:of\s+(?:the\s+)?)?(?:work(?:ing)?|scheduled))\s*)?[:=\-–—]?\s*(\d{1,2}(?:\.\d+)?)\+?\s*(?:(?:per(?:\s+(?:the|each|every))|p(?:er[\s.]*)?\.?\s*/?|/\s*(?:(?:the|each|every)\s+)?|an?\.?\s*/?|for(?:\s+(?:the|each|every))?|of(?:\s+(?:the|each|every))?|each|every)\s*)?(?:working[\s-]+(?:weekly|wkly|weeks?|wk)|weekly|wkly|weeks?|wk|w)\.?\b"
