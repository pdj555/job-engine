...
_NON_SALARY_MONEY_RE = re.compile(
    ...
    r"(?:\s*(?:[-–—]|to)\s*(?:USD|US\$|\$)?\s*[\d,]+(?:\s+\d{3})*(?:\.\d+)?(?:\s*k)?)?"
    r"(?:" + _ANY_PERIOD + r")?"
    r"\s+(?:HSA|FSA|HRA|QSEHRA|ICHRA|pension|529|401(?:k|\(k\))|403(?:b|\(b\))|457(?:[bf]|\([bf]\))|(?:SEP|SIMPLE)\s+IRA|SIMPLE\s+401(?:k|\(k\))|solo\s+401(?:k|\(k\))|Roth\s+(?:IRA|401(?:k|\(k\))|403(?:b|\(b\)))|QACA|safe\s+harbor(?:\s+match)?|DCFSA|LFSA|LPFSA|HCFSA|VEBA|DCAP|Keogh|COBRA|(?:IRC|Sec(?:tion)?\.?)\s*125(?:\s+plan)?|cafeteria\s+(?:plan|benefit)|POP\s+plan|OOP|flex(?:ible)?\s+spending(?:\s+account)?|health\s+savings\s+account|health\s+reimbursement\s+arrangement)(?:\s+(?:contribution|benefit|account|fund|credit|rebate|max|cap|offset|clawback|pre[- ]?tax|catch[- ]?up|carry[- ]?over|roll[- ]?over|true[- ]?up|makeup))?+(?:\s+(?:recoupment|recovery|repayment|holdback))?+(?:\b|(?<=\)))(?!\s+in)(?!\s*(?:(?:match|matching|contribution|benefits?|account|fund|credit|rebate|max|cap|offset|clawback|pre[- ]?tax|catch[- ]?up|carry[- ]?over|roll[- ]?over|true[- ]?up|makeup)\s+)?(?:of|:)?\s*(?:USD|US\$|\$)\s*[\d,])"
    r"|"
    r"\b(?:HSA|FSA|HRA|QSEHRA|ICHRA|pension|529|401(?:k|\(k\))|403(?:b|\(b\))|457(?:[bf]|\([bf]\))|(?:SEP|SIMPLE)\s+IRA|SIMPLE\s+401(?:k|\(k\))|solo\s+401(?:k|\(k\))|Roth\s+(?:IRA|401(?:k|\(k\))|403(?:b|\(b\)))|QACA|safe\s+harbor(?:\s+match)?|DCFSA|LFSA|LPFSA|HCFSA|VEBA|DCAP|Keogh|COBRA|(?:IRC|Sec(?:tion)?\.?)\s*125(?:\s+plan)?|cafeteria\s+(?:plan|benefit)|POP\s+plan|OOP|flex(?:ible)?\s+spending(?:\s+account)?|health\s+savings\s+account|health\s+reimbursement\s+arrangement)(?:\s+(?:contribution|benefit|account|fund|credit|rebate|max|cap|offset|clawback|pre[- ]?tax|catch[- ]?up|carry[- ]?over|roll[- ]?over|true[- ]?up|makeup))?\s*(?:of|:)?\s*"
    r"(?:USD|US\$|\$)\s*[\d,]+(?:\s+\d{3})*(?:\.\d+)?(?:\s*k)?"
    r"(?:\s*(?:[-–—]|to)\s*(?:USD|US\$|\$)?\s*[\d,]+(?:\s+\d{3})*(?:\.\d+)?(?:\s*k)?)?"
    r"|"
    r"(?:USD|US\$|\$)\s*[\d,]+(?:\s+\d{3})*(?:\.\d+)?(?:\s*k)?"
