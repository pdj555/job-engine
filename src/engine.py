...
_ONSITE_WORKPLACE_RE = re.compile(
    ...
    r"|\breport\s+(?:in\s+)?(?:to|at)\s+(?:the\s+|our\s+|an\s+)?(?:\S+\s+){0,4}(?<!home )(?<!home-)(?:offices?|campus|lab(?:oratory)?|headquarters|hq|hub)\b"
    ...
    r"|\bcome\s+(?:in(?:to|\s+to)|to)\s+(?:the\s+|our\s+|an\s+)?(?:\S+\s+){0,4}(?<!home )(?<!home-)(?:offices?|campus|lab(?:oratory)?|headquarters|hq|hub)\b(?!\s+hours)"
