...
_NON_SALARY_MONEY_RE = re.compile(
    ...
    r"(?:" + _ANY_PERIOD + r")?"
    r"\s+(?:in\s+)?(?:equity|stock(?:\s+options?)?+|RSUs?|ESPP(?:\s+(?:contribution|match|discount))?+|ESOPs?(?:\s+(?:grant|award|contribution|clawback))?+|employee\s+stock(?:\s+(?:purchase|ownership)(?:\s+plan)?)?+|restricted\s+stock|option\s+grant|severance)\b(?!\s*(?:(?:contribution|grant|match|discount|purchase|clawback|rebate|offset|recoupment|repayment|recovery|holdback)\s+)?(?:of|:)?\s*(?:USD|US\$|\$)\s*[\d,])"
    r"|"
    r"\b(?:equity|stock(?:\s+options?)?|RSUs?|ESPP(?:\s+(?:contribution|match|discount))?|ESOPs?(?:\s+(?:grant|award|contribution))?|employee\s+stock(?:\s+(?:purchase|ownership)(?:\s+plan)?)?|restricted\s+stock|option\s+grant|severance)\s*(?:of|:)?\s*"
    r"(?:USD|US\$|\$)\s*[\d,]+(?:\s+\d{3})*(?:\.\d+)?(?:\s*k)?"
    r"(?:\s*(?:[-–—]|to)\s*(?:USD|US\$|\$)?\s*[\d,]+(?:\s+\d{3})*(?:\.\d+)?(?:\s*k)?)?"
    r"|"
    r"(?:USD|US\$|\$)\s*[\d,]+(?:\s+\d{3})*(?:\.\d+)?(?:\s*k)?"
...
_ONSITE_WORKPLACE_RE = re.compile(
    ...
    r"|\breport\s+(?:to|at)\s+(?:the\s+|our\s+|an\s+)?(?:\S+\s+){0,4}offices?\b"
    ...
    r"|\breport\s+(?:to|at)\s+(?:the\s+|our\s+|an\s+)?(?:headquarters|HQ)\s+"
    r"(?:two|three|four|five|\d+(?:\s*[-–]\s*\d+)?)\s+days?(?:\s+(?:(?:a|per)\s+week|weekly)|\s*/\s*weeks?)\b"
    r"|\breport\s+(?:to|at)\s+"
