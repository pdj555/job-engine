...
_GONE_LISTING_RE = re.compile(
    ...
    r"|(?<!once )(?<!after )(?<!when )(?:the|this)\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening|search)"
    r"\s+is\s+no\s+longer\s+being\s+(?:staffed|filled|funded|pursued(?!\s+as\b)|considered(?!\s+for\b))\b"
...
_DAILY_HOURS_RE = re.compile(
    r"(?<![\d.])(\d{1,2}(?:\.\d+)?)\+?[\s-]*(?:hours?|hrs?)\.?\s*"
    r"(?:/\s*|\s*per[\s./]*|\s+p\.?\s*/\s*|\s+(?:a|each|every)\s+)\s*"
    r"(?:working\s+days?|business\s+days?|work\s+days?|workdays?|weekdays?|days?)\b"
    r"(?!\s+(?:meeting|standup|stand-up|sync|call|all-?hands))"
    r"|(?<![\d.])(\d{1,2}(?:\.\d+)?)\+?[\s-]*(?:hours?|hrs?)\.?\s+daily\b"
    r"(?!\s+(?:meeting|standup|stand-up|sync|call|all-?hands))"
    r"|(?:hours?|hrs?)\s+(?:(?:per|a|each|every|/)\s+)(?:working\s+days?|business\s+days?|work\s+days?|workdays?|weekdays?|days?)\s*[:=\-–—]?\s*(\d{1,2}(?:\.\d+)?)\+?"
    ...
    r"|(?:hours?|hrs?)\s*[:=\-–—]\s*(\d{1,2}(?:\.\d+)?)\+?\s*"
    r"(?:(?:per|a|each|every|/)\s+)(?:working\s+days?|business\s+days?|work\s+days?|workdays?|weekdays?|days?)\b"
...
def _stated_hours(title: str, description: str) -> Optional[int]:
    """Hours explicitly written as N hours/week, N hours/fortnight halved, N hours 2–8 times a week, or N hours/day, workday, or business day × 5."""
