...
_HOURS_RE = re.compile(
    ...
    r"|(?:hours?|hrs?)\s*(?:(?:worked|working|scheduled)\s+)?(?:(?:per[\s./]*|/|a|for(?:\s+the)?)\s*)?(?:wk|weeks?|weekly)\s*(?:scheduled\s*)?[:=\-–—]?\s*(\d{1,2}(?:\.\d+)?)\+?"
    ...
    r"(?!\s+(?:meeting|standup|stand-up|sync|call|all-?hands))"
    r"|(?:hours?|hrs?)\s*(?:worked\s*)?[:=\-–—]\s*(\d{1,2}(?:\.\d+)?)\+?\s*(?:per[\s./]*|/|a\s+)(?:wk|weeks?|weekly)\b"
