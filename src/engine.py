...
_NON_SALARY_MONEY_RE = re.compile(
    ...
    r"(?:\s*(?:[-–—]|to)\s*(?:USD|US\$|\$)?\s*[\d,]+(?:\s+\d{3})*(?:\.\d+)?(?:\s*k)?)?"
    r"(?:" + _ANY_PERIOD + r")?"
    r"\s+(?:(?:ORCA|Clipper|Ventra|TAP|OMNY)\s+card|metro(?:\s+)?cards?|fare(?:\s+)?cards?|Hop\s+Fast[-\s]?Pass|E[- ]?Z[- ]?Pass|Charlie(?:\s+)?Card|SmarTrip)\b(?!\s+in)(?!\s*(?:of|:)?\s*(?:USD|US\$|\$)\s*[\d,])"
    r"|"
    r"\b(?:(?:ORCA|Clipper|Ventra|TAP|OMNY)\s+card|metro(?:\s+)?cards?|fare(?:\s+)?cards?|Hop\s+Fast[-\s]?Pass|E[- ]?Z[- ]?Pass|Charlie(?:\s+)?Card|SmarTrip)\s*(?:of|:)?\s*"
    r"(?:USD|US\$|\$)\s*[\d,]+(?:\s+\d{3})*(?:\.\d+)?(?:\s*k)?"
    r"(?:\s*(?:[-–—]|to)\s*(?:USD|US\$|\$)?\s*[\d,]+(?:\s+\d{3})*(?:\.\d+)?(?:\s*k)?)?"
    r"|"
    r"(?:USD|US\$|\$)\s*[\d,]+(?:\s+\d{3})*(?:\.\d+)?(?:\s*k)?"
