...
_ONSITE_WORKPLACE_RE = re.compile(
    ...
    r"(?:\s+(?!two\b|three\b|four\b|five\b|\d+\b)\S+){0,4}"
    r"\s+(?:two|three|four|five|\d+(?:\s*[-–]\s*\d+)?)\s+days?\b"
    r"|(?:must|should(?!n't)(?!\s+not)|(?<!not )(?<!n't )ha(?:ve|s|d)\s+to)\s+(?:sit|remain|stay)\s+(?:in|at)\s+"
    ...
    r"|(?<!not )(?:required|expected|expects?|need(?:s|ed)?|asked|requested|requests?|invited|encouraged|preferred|prefer|supposed|ought|instructed|directed|mandated|obligated|compelled|ordered|told)\s+to\s+(?:sit|remain|stay)\s+(?:in|at)\s+"
    r"(?:the\s+|our\s+|an\s+)?(?:\S+\s+){0,4}(?<!home )(?<!home-)(?:offices?|campus|lab(?:oratory)?|headquarters|hq)\b"
