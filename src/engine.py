...
_NON_SALARY_MONEY_RE = re.compile(
    ...
    r"(?:\s*(?:[-–—]|to)\s*(?:USD|US\$|\$)?\s*[\d,]+(?:\s+\d{3})*(?:\.\d+)?(?:\s*k)?)?"
    r"(?:" + _ANY_PERIOD + r")?"
    r"\s+(?:Citi[-\s]?bikes?|Blue[-\s]?bikes?|Divvy[-\s]?bikes?|Lime[-\s]?bikes?|Jump[-\s]?bikes?|(?:(?!in\b)\S+\s+)?bike[-\s]?share)\b(?!\s+in)(?!\s*(?:of|:)?\s*(?:USD|US\$|\$)\s*[\d,])"
    r"|"
    r"\b(?:Citi[-\s]?bikes?|Blue[-\s]?bikes?|Divvy[-\s]?bikes?|Lime[-\s]?bikes?|Jump[-\s]?bikes?|(?:(?!in\b)\S+\s+)?bike[-\s]?share)\s*(?:of|:)?\s*"
    r"(?:USD|US\$|\$)\s*[\d,]+(?:\s+\d{3})*(?:\.\d+)?(?:\s*k)?"
    r"(?:\s*(?:[-–—]|to)\s*(?:USD|US\$|\$)?\s*[\d,]+(?:\s+\d{3})*(?:\.\d+)?(?:\s*k)?)?"
    r"|"
    r"(?:USD|US\$|\$)\s*[\d,]+(?:\s+\d{3})*(?:\.\d+)?(?:\s*k)?"
    ...
    r"(?:\s*(?:[-–—]|to)\s*(?:USD|US\$|\$)?\s*[\d,]+(?:\s+\d{3})*(?:\.\d+)?(?:\s*k)?)?"
    r"(?:" + _ANY_PERIOD + r")?"
    r"\s+(?:GTL|(?:group(?:\s+term)?|supplemental|basic|voluntary|term|whole|universal|(?:variable|indexed)(?:\s+universal)?|buy[-\s]sell)\s+life(?:\s+insurance)?|split[-\s]dollar(?:\s+(?:life|plan|insurance))?|key(?:[-\s]?person|[-\s]man)\s+(?:life|insurance)|irrevocable\s+life(?:\s+insurance(?:\s+trust)?)?|(?:corporate|bank|company|employer)[-\s]owned\s+life)(?:\s+(?:benefit|coverage|offset|rebate|max|cap|insurance))?+(?:\s+(?:clawback|recoupment|recovery|repayment|holdback))?+\b(?!\s+in)(?!\s*(?:of|:)?\s*(?:USD|US\$|\$)\s*[\d,])"
    r"|"
    r"\b(?:GTL|(?:group(?:\s+term)?|supplemental|basic|voluntary|term|whole|universal|(?:variable|indexed)(?:\s+universal)?|buy[-\s]sell)\s+life(?:\s+insurance)?|split[-\s]dollar(?:\s+(?:life|plan|insurance))?|key(?:[-\s]?person|[-\s]man)\s+(?:life|insurance)|irrevocable\s+life(?:\s+insurance(?:\s+trust)?)?|(?:corporate|bank|company|employer)[-\s]owned\s+life)\s*(?:of|:)?\s*"
    r"(?:USD|US\$|\$)\s*[\d,]+(?:\s+\d{3})*(?:\.\d+)?(?:\s*k)?"
    r"(?:\s*(?:[-–—]|to)\s*(?:USD|US\$|\$)?\s*[\d,]+(?:\s+\d{3})*(?:\.\d+)?(?:\s*k)?)?"
    r"|"
    r"(?:USD|US\$|\$)\s*[\d,]+(?:\s+\d{3})*(?:\.\d+)?(?:\s*k)?"
...
_ONSITE_WORKPLACE_RE = re.compile(
    ...
    r"|\bwork\s+(?:at|on)\s+(?:the\s+|our\s+|an\s+)?(?:\S+\s+){0,4}(?<!home )(?<!home-)(?:offices?|campus|lab(?:oratory)?|headquarters|hq|hub|(?<!off-)(?<!off )site|field(?!\s+of\b))\b"
    ...
    r"|\breport\s+(?:in\s+)?(?:to|at|on)\s+(?:the\s+|our\s+|an\s+)?(?:\S+\s+){0,4}(?<!home )(?<!home-)(?:offices?|campus|lab(?:oratory)?|headquarters|hq|hub|(?<!off-)(?<!off )site|field(?!\s+of\b))\b"
    ...
    r"|\bcome\s+(?:in(?:to|\s+to)|to|on)\s+(?:the\s+|our\s+|an\s+)?(?:\S+\s+){0,4}(?<!home )(?<!home-)(?:offices?|campus|lab(?:oratory)?|headquarters|hq|hub|(?<!off-)(?<!off )site|field(?!\s+of\b))\b(?!\s+hours)"
    ...
    r"|(?:must|should(?!n't)(?!\s+not)|(?<!not )(?<!n't )ha(?:ve|s|d)\s+to)\s+be\s+(?:(?:located|stationed|based|seated|present)\s+)?(?:in|at|on)\s+"
    ...
    r"|(?<!not )(?:required|expected|expects?|need(?:s|ed)?|asked|requested|requests?|invited|encouraged|preferred|prefer|supposed|ought|instructed|directed|mandated|obligated|compelled|ordered|told)\s+to\s+be\s+(?:(?:located|stationed|based|seated|present)\s+)?(?:in|at|on)\s+"
    r"(?:the\s+|our\s+|an\s+)?(?:\S+\s+){0,4}(?<!home )(?<!home-)(?:offices?|campus|lab(?:oratory)?|headquarters|hq|hub|(?<!off-)(?<!off )site|field(?!\s+of\b))\b"
    ...
    r"(?:\s+(?!two\b|three\b|four\b|five\b|\d+\b)\S+){0,4}"
    r"\s+(?:two|three|four|five|\d+(?:\s*[-–]\s*\d+)?)\s+days?\b"
    r"|(?:two|three|four|five|\d+(?:\s*[-–]\s*\d+)?)\s+days?(?:\s+(?:(?:a|per)\s+week|weekly)|\s*/\s*weeks?)?\s+(?:in|on)\s+"
    ...
    r"(?:\s+(?!two\b|three\b|four\b|five\b|\d+\b)\S+){0,4}"
    r"\s+(?:two|three|four|five|\d+(?:\s*[-–]\s*\d+)?)\s+days?\b"
    r"|(?:must|should(?!n't)(?!\s+not)|(?<!not )(?<!n't )ha(?:ve|s|d)\s+to)\s+(?:sit|remain|stay)\s+(?:in|at|on|out\s+of)\s+"
    r"(?:the\s+|our\s+|an\s+)?(?:\S+\s+){0,4}(?<!home )(?<!home-)(?:offices?|campus|lab(?:oratory)?|headquarters|hq|hub|(?<!off-)(?<!off )site|field(?!\s+of\b))\b"
    r"|(?:must|should(?!n't)(?!\s+not)|(?<!not )(?<!n't )ha(?:ve|s|d)\s+to)\s+(?:sit|remain|stay)\s+on[-\s]campus\b"
    r"|(?<!not )(?:required|expected|expects?|need(?:s|ed)?|asked|requested|requests?|invited|encouraged|preferred|prefer|supposed|ought|instructed|directed|mandated|obligated|compelled|ordered|told)\s+to\s+(?:sit|remain|stay)\s+(?:in|at|on|out\s+of)\s+"
    r"(?:the\s+|our\s+|an\s+)?(?:\S+\s+){0,4}(?<!home )(?<!home-)(?:offices?|campus|lab(?:oratory)?|headquarters|hq|hub|(?<!off-)(?<!off )site|field(?!\s+of\b))\b"
    ...
    r"|(?:must|should(?!n't)(?!\s+not)|(?<!not )(?<!n't )ha(?:ve|s|d)\s+to)\s+(?:clock|check|sign|punch|badge|swipe|tap|scan)\s+(?:in|out|on|off)\s+(?:at|on)\s+"
    r"(?:the\s+|our\s+|an\s+)?(?:\S+\s+){0,4}(?<!home )(?<!home-)(?:offices?|campus|lab(?:oratory)?|headquarters|hq|hub|(?<!off-)(?<!off )site|field(?!\s+of\b))\b"
    r"|(?<!not )(?:required|expected|expects?|need(?:s|ed)?|asked|requested|requests?|invited|encouraged|preferred|prefer|supposed|ought|instructed|directed|mandated|obligated|compelled|ordered|told)\s+to\s+(?:clock|check|sign|punch|badge|swipe|tap|scan)\s+(?:in|out|on|off)\s+(?:at|on)\s+"
    r"(?:the\s+|our\s+|an\s+)?(?:\S+\s+){0,4}(?<!home )(?<!home-)(?:offices?|campus|lab(?:oratory)?|headquarters|hq|hub|(?<!off-)(?<!off )site|field(?!\s+of\b))\b"
