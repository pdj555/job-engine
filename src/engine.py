...
_NON_SALARY_MONEY_RE = re.compile(
    ...
    r"|"
    r"\b(?:mileage|clothing|equipment|laptop|moving|uniform|bike|coworking|adoption|fertility|parental|WFH|tools|EAP|sabbatical|PTO|bereavement|options|copay|deductible|coinsurance|out(?:-|\s+)of(?:-|\s+)pocket|subsidy|perk|voucher|discount|cellphone|wifi|wi-fi|broadband)\s*(?:of|:)?\s*"
    r"(?:USD|US\$|\$)\s*[\d,]+(?:\.\d+)?(?:\s*k)?"
    r"(?:\s*(?:[-–—]|to)\s*(?:USD|US\$|\$)?\s*[\d,]+(?:\.\d+)?(?:\s*k)?)?"
    r"|"
