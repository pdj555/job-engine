...
_NON_SALARY_MONEY_RE = re.compile(
    ...
    r"(?:" + _ANY_PERIOD + r")?"
    r"\s+(?:in\s+)?(?:equity|stock(?:\s+options?)?+|RSUs?|ESPP(?:\s+(?:contribution|match|discount))?+|employee\s+stock(?:\s+purchase(?:\s+plan)?)?+|restricted\s+stock|option\s+grant|severance)\b(?!\s*(?:(?:contribution|grant|match|discount|purchase|clawback|rebate|offset|recoupment|repayment|recovery|holdback)\s+)?(?:of|:)?\s*(?:USD|US\$|\$)\s*[\d,])"
    r"|"
    r"\b(?:equity|stock(?:\s+options?)?|RSUs?|ESPP(?:\s+(?:contribution|match|discount))?|employee\s+stock(?:\s+purchase(?:\s+plan)?)?|restricted\s+stock|option\s+grant|severance)\s*(?:of|:)?\s*"
    r"(?:USD|US\$|\$)\s*[\d,]+(?:\s+\d{3})*(?:\.\d+)?(?:\s*k)?"
    r"(?:\s*(?:[-–—]|to)\s*(?:USD|US\$|\$)?\s*[\d,]+(?:\s+\d{3})*(?:\.\d+)?(?:\s*k)?)?"
    r"|"
    r"(?:USD|US\$|\$)\s*[\d,]+(?:\s+\d{3})*(?:\.\d+)?(?:\s*k)?"
    ...
    r"(?:\s*(?:[-–—]|to)\s*(?:USD|US\$|\$)?\s*[\d,]+(?:\s+\d{3})*(?:\.\d+)?(?:\s*k)?)?"
    r"(?:" + _ANY_PERIOD + r")?"
    r"\s+(?:NQSOs?|NSOs?|ISOs?|RSAs?|DSUs?|performance\s+(?:units|shares|stock)|restricted\s+(?:units|shares)|phantom\s+(?:stock|units|shares)|deferred\s+(?:units|shares|stock)|incentive\s+stock(?:\s+options?)?|non[- ]?qualified\s+(?:stock(?:\s+options?)?|options)|(?:share|option)\s+warrants?|warrants?)(?:\s+(?:grant|award|clawback|recoupment|recovery|repayment|holdback))?+\b(?!\s+in)(?!\s*(?:of|:)?\s*(?:USD|US\$|\$)\s*[\d,])"
    r"|"
    r"\b(?:NQSOs?|NSOs?|ISOs?|RSAs?|DSUs?|performance\s+(?:units|shares|stock)|restricted\s+(?:units|shares)|phantom\s+(?:stock|units|shares)|deferred\s+(?:units|shares|stock)|incentive\s+stock(?:\s+options?)?|non[- ]?qualified\s+(?:stock(?:\s+options?)?|options)|(?:share|option)\s+warrants?|warrants?)\s*(?:of|:)?\s*"
    r"(?:USD|US\$|\$)\s*[\d,]+(?:\s+\d{3})*(?:\.\d+)?(?:\s*k)?"
    r"(?:\s*(?:[-–—]|to)\s*(?:USD|US\$|\$)?\s*[\d,]+(?:\s+\d{3})*(?:\.\d+)?(?:\s*k)?)?"
    r"|"
    r"(?:USD|US\$|\$)\s*[\d,]+(?:\s+\d{3})*(?:\.\d+)?(?:\s*k)?"
