...
_NON_SALARY_MONEY_RE = re.compile(
    ...
    r"(?:\s+(?:housing|living|relocation|meal|food|travel|wellness|phone|cell|internet|commuter|parking|home\s+office|gym(?:\s+membership)?|fitness))*"
    r"\s+stipend\b"
    r"|"
    ...
    r"(?:\s*(?:[-–—]|to)\s*(?:USD|US\$|\$)?\s*[\d,]+(?:\.\d+)?(?:\s*k)?)?"
    r"(?:\s*(?:/\s*mo(?:nth)?s?|(?:per|a)\s+mo(?:nth)?s?|monthly))?"
    r"(?:\s+(?:housing|living|relocation|car|vehicle|auto|phone|cell|internet|meal|food|travel|commuter|parking|home\s+office|gym(?:\s+membership)?|fitness))*"
    r"\s+allowance\b"
    r"|"
