...
_FOREIGN_PAY_RE = re.compile(
    ...
    r"|(?:₹|¥|₩|₪|฿|₫|₦|₴|₸|₺|₽|₱|৳|₾|₼|₡|₭|៛)\s*\d"
    r"|\b(?:CHF|INR|JPY|AED|CNY|KRW|HUF|SEK|NOK|DKK|PLN|BRL|ZAR|ILS|CZK|TRY|RON|BGN|THB|TWD|MYR|IDR|RUB|UAH|ISK|HRK|VND|NGN|PKR|BDT|LKR|SAR|QAR|EGP|KES|GHS|KWD|BHD|OMR|JOD|RSD|GEL|KZT|TND|DZD|BAM|AZN|MDL|MKD|BYN|NPR|UZS|KGS|MMK|KHR|LAK|MNT|UYU|PYG|BOB|GTQ|IQD|LBP|TZS|UGX|ETB|RWF|BND)\s*['’]?\d"
    ...
    r"|\d{1,3}(?:[,'’.\s]\d{3}){1,2}\s*(?:CHF|INR|JPY|AED|CNY|KRW|HUF|SEK|NOK|DKK|PLN|BRL|ZAR|ILS|CZK|TRY|RON|BGN|THB|TWD|PHP|MYR|IDR|RUB|UAH|ISK|HRK|VND|NGN|PKR|BDT|LKR|SAR|QAR|EGP|KES|GHS|KWD|BHD|OMR|JOD|RSD|GEL|KZT|TND|DZD|BAM|AZN|MDL|MKD|BYN|NPR|UZS|KGS|MMK|KHR|LAK|MNT|UYU|PYG|BOB|GTQ|IQD|LBP|TZS|UGX|ETB|RWF|BND)\b"
    r"|\d{5,7}\s*(?:CHF|INR|JPY|AED|CNY|KRW|HUF|SEK|NOK|DKK|PLN|BRL|ZAR|ILS|CZK|TRY|RON|BGN|THB|TWD|PHP|MYR|IDR|RUB|UAH|ISK|HRK|VND|NGN|PKR|BDT|LKR|SAR|QAR|EGP|KES|GHS|KWD|BHD|OMR|JOD|RSD|GEL|KZT|TND|DZD|BAM|AZN|MDL|MKD|BYN|NPR|UZS|KGS|MMK|KHR|LAK|MNT|UYU|PYG|BOB|GTQ|IQD|LBP|TZS|UGX|ETB|RWF|BND)\b"
    r"|\d{2,3}(?:\.\d+)?\s*k\s*(?:CHF|INR|JPY|AED|CNY|KRW|HUF|SEK|NOK|DKK|PLN|BRL|ZAR|ILS|CZK|TRY|RON|BGN|THB|TWD|PHP|MYR|IDR|RUB|UAH|ISK|HRK|VND|NGN|PKR|BDT|LKR|SAR|QAR|EGP|KES|GHS|KWD|BHD|OMR|JOD|RSD|GEL|KZT|TND|DZD|BAM|AZN|MDL|MKD|BYN|NPR|UZS|KGS|MMK|KHR|LAK|MNT|UYU|PYG|BOB|GTQ|IQD|LBP|TZS|UGX|ETB|RWF|BND)\b"
...
_GONE_LISTING_RE = re.compile(
    ...
    r"\s+is\s+no\s+longer\s+(?:available|active|posted|listed|live|published|advertised)"
    ...
    r"|(?<!once )(?<!after )(?<!when )(?:the|this)\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening|search)"
    r"\s+no\s+longer\s+(?:available|active|posted|listed|live|published|advertised)\b"
    r"|(?<!once )(?<!after )(?<!when )(?<!this )(?<!the )(?<!a )(?:sorry,\s+)?"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening)"
    r"\s+no\s+longer\s+(?:available|active|posted|listed|live|published|advertised)\b"
    r"|(?:the|this)\s+"
    ...
    r"|(?<!once )(?<!after )(?<!when )(?:the|this)\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening|search)"
    r"\s+has\s+(?:been\s+)?(?:filled|expired|closed|withdrawn|cancelled|canceled|taken\s+down|taken\s+offline|ended(?!\s+up)|removed|discontinued|archived|unpublished|deactivated|deleted|unposted|pulled)"
    r"|(?<!once )(?<!after )(?<!when )(?<!this )(?<!the )(?<!a )(?:sorry,\s+)?"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening)"
    r"\s+has\s+(?:been\s+)?(?:filled|expired|closed|withdrawn|cancelled|canceled|taken\s+down|taken\s+offline|ended(?!\s+up)|removed|discontinued|archived|unpublished|deactivated|deleted|unposted|pulled)"
    r"|(?<!once )(?<!after )(?<!when )(?:the|this)\s+"
    ...
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening|search)"
    r"|we(?:(?:'re| are)\s+not| aren't)\s+filling\s+this\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening|search)"
    r"|(?<!once )(?<!after )(?<!when )(?:the|this)\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening|search)"
    ...
    r"|hiring\s+has\s+closed\s+for\s+this\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening|search)"
