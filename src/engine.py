...
_GONE_LISTING_RE = re.compile(
    ...
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening)"
    r"\s+is\s+(?:(?:un|de[-\s]?)published|(?:un|de[-\s]?)released|(?:un|de[-\s]?)syndicated|(?:un|de[-\s]?)promoted|(?:un|de[-\s]?)featured|(?:un|de[-\s]?)broadcast|(?:un|de[-\s]?)indexed|(?:un|de[-\s]?)circulated|(?:un|de[-\s]?)shared|(?:un|de[-\s]?)boosted|(?:un|de[-\s]?)sponsored|(?:un|de[-\s]?)endorsed|(?:un|de[-\s]?)advertised|(?:un|de[-\s]?)posted|archived|(?:un|de[-\s]?)activated)\b"
    r"|we(?:'re| are)\s+no\s+longer\s+(?:hiring\s+for\s+this|recruiting\s+for\s+this|advertising\s+for\s+this|(?:accepting|taking|receiving|collecting|processing|evaluating|assessing|screening|shortlisting|interviewing|seeking|sourcing|reviewing|inviting|welcoming|soliciting|requesting|encouraging|onboarding|enrolling|registering|contacting|scheduling)\s+(?:new\s+)?(?:applications|applicants)\b(?!\s+from))"
    ...
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening|search)"
    r"|we(?:'ve|\s+have)?\s+stopped\s+(?:accepting|taking|receiving|collecting|processing|considering|evaluating|assessing|screening|shortlisting|interviewing|reviewing|seeking|sourcing|inviting|welcoming|soliciting|requesting|encouraging|onboarding|enrolling|registering|contacting|scheduling)\s+(?:new\s+)?(?:applications|applicants)\b(?!\s+from)"
    ...
    r"|we(?:(?:'re| are)\s+not| aren't)\s+(?:accepting|taking|receiving|collecting|processing|evaluating|assessing|screening|shortlisting|interviewing|seeking|sourcing|reviewing|inviting|welcoming|soliciting|requesting|encouraging|onboarding|enrolling|registering|contacting|scheduling)\s+(?:new\s+)?(?:applications|applicants)\s+for\s+this\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening|search)"
    r"|no\s+longer\s+(?:accepting|taking|receiving|collecting|processing|considering|evaluating|assessing|screening|shortlisting|interviewing|seeking|sourcing|reviewing|inviting|welcoming|soliciting|requesting|encouraging|onboarding|enrolling|registering|contacting|scheduling)\s+(?:new\s+)?(?:applications|applicants|candidates)\b(?!\s+from)"
    ...
    r"|(?<!once )(?<!after )(?<!when )(?:the|this)\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening|search)"
    r"\s+is\s+no\s+longer\s+being\s+(?:staffed|filled|pursued(?!\s+as\b)|considered(?!\s+for\b))\b"
...
_HOURS_RE = re.compile(
    ...
    r"|(?:(?<!twice )(?<!thrice )(?<!times )weekly|wkly|(?<!other )(?<!two )(?<!twice a )(?<!times a )weeks?|wk)\s+(?:(?:scheduled|work(?:ing)?)\s+)*(?:hours?|hrs?)(?:\s+(?:of\s+(?:the\s+)?)?(?:work(?:ing)?|scheduled))?\.?\s*(?:scheduled\s*)?[:=\-–—]?\s*(\d{1,2}(?:\.\d+)?)\+?"
    ...
    r"(?!\s+(?:meeting|standup|stand-up|sync|call|all-?hands))"
    r"|(?<![\d.])(\d{1,2}(?:\.\d+)?)\+?[\s-]*(?:scheduled[\s-]+)?(?:weekly|wkly|weeks?|wk)[\s-]+(?:(?:scheduled|work(?:ing)?)[\s-]+)?(?:hours?|hrs?)\b"
    r"(?!\s+(?:meeting|standup|stand-up|sync|call|all-?hands))"
...
_FORTNIGHT_HOURS_RE = re.compile(
    ...
    r"(?:working\s+fortnights?|fortnights?|two\s+weeks)\b"
    r"(?!\s+(?:meeting|standup|stand-up|sync|call|all-?hands))"
    r"|(?<![\d.])(\d{1,2}(?:\.\d+)?)\+?[\s-]*(?:hours?|hrs?)\.?\s+fortnightly\b"
    r"(?!\s+(?:meeting|standup|stand-up|sync|call|all-?hands))"
    r"|(?:hours?|hrs?)\s+(?:(?:per|a|each|every|/)\s+)(?:working\s+fortnights?|fortnights?|two\s+weeks)\s*[:=\-–—]?\s*(\d{1,2}(?:\.\d+)?)\+?"
    ...
    r"(?:(?:per|a|each|every|/)\s+)(?:working\s+fortnights?|fortnights?|two\s+weeks)\b"
...
def _stated_hours(title: str, description: str) -> Optional[int]:
    """Hours explicitly written as N hours/week, N hours/fortnight halved, N hours 2–5 times a week, or N hours/day × 5."""
