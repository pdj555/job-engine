...
_HOURS_RE = re.compile(
    ...
    r"|(?:weekly|wkly)\s+(?:(?:scheduled|work(?:ing)?)\s+)*(?:hours?|hrs?)\s*(?:scheduled\s*)?[:=\-–—]?\s*(\d{1,2}(?:\.\d+)?)\+?"
    r"|(?:hours?|hrs?)\s+weekly\s*[:=\-–—]?\s*(\d{1,2}(?:\.\d+)?)\+?"
    r"|(?<![\d.])(\d{1,2}(?:\.\d+)?)\+?[\s-]*(?:scheduled[\s-]+)?(?:weekly|wkly)[\s-]+(?:(?:scheduled|work(?:ing)?)[\s-]+)?(?:hours?|hrs?)\b"
    r"(?!\s+(?:meeting|standup|stand-up|sync|call|all-?hands))"
    r"|(?:hours?|hrs?)\s*(?:worked\s*)?[:=\-–—]\s*(\d{1,2}(?:\.\d+)?)\+?\s*(?:(?:p(?:er[\s.]*)?\.?\s*/?|/|an?\.?\s*/?|each|every)\s*)?(?:wk|weeks?|weekly|wkly)\b"
