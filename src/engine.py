...
_HOURS_RE = re.compile(
    ...
    r"|\b(?:(?:per|each|every|for(?:\s+(?:the|each|every))?)\s+(?:weekly|wkly|weeks?|wk)|weekly|wkly|p(?:/|\.)(?:weekly|wkly|weeks?|wk|w)\.?|pwk|pw)\s*[:=\-–—]?\s*(\d{1,2}(?:\.\d+)?)\+?\s*(?:(?:scheduled|work(?:ing)?)\s+)?(?:hours?|hrs?)\b"
    r"(?!\s+(?:meeting|standup|stand-up|sync|call|all-?hands))"
