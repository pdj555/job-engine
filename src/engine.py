...
_NON_SALARY_MONEY_RE = re.compile(
    ...
    r"(?:\s*(?:[-–—]|to)\s*(?:USD|US\$|\$)?\s*[\d,]+(?:\s+\d{3})*(?:\.\d+)?(?:\s*k)?)?"
    r"(?:" + _HOUR_TAIL + r"|" + _DAY_TAIL + r"|" + _WEEK_TAIL + r"|" + _MONTH_TAIL + r"|" + _BIWEEK_TAIL + r"|" + _SEMIMONTH_TAIL + r")?"
    r"\s+(?:bicycle|cycling|e-?bikes?)\s+(?:benefit|reimbursement|perks?)\b(?!\s+in)(?!\s*(?:of|:)?\s*(?:USD|US\$|\$)\s*[\d,])"
    r"|"
    r"\b(?:bicycle|cycling|e-?bikes?)\s+(?:benefit|reimbursement|perks?)\s*(?:of|:)?\s*"
    r"(?:USD|US\$|\$)\s*[\d,]+(?:\s+\d{3})*(?:\.\d+)?(?:\s*k)?"
    r"(?:\s*(?:[-–—]|to)\s*(?:USD|US\$|\$)?\s*[\d,]+(?:\s+\d{3})*(?:\.\d+)?(?:\s*k)?)?"
    r"|"
    r"(?:USD|US\$|\$)\s*[\d,]+(?:\s+\d{3})*(?:\.\d+)?(?:\s*k)?"
    r"(?:\s*(?:[-–—]|to)\s*(?:USD|US\$|\$)?\s*[\d,]+(?:\s+\d{3})*(?:\.\d+)?(?:\s*k)?)?"
    r"(?:" + _HOUR_TAIL + r"|" + _DAY_TAIL + r"|" + _WEEK_TAIL + r"|" + _MONTH_TAIL + r"|" + _BIWEEK_TAIL + r"|" + _SEMIMONTH_TAIL + r")?"
    r"(?:\s+(?:housing|living|relocation|car|vehicle|auto|phone|cell(?:phone)?|mobile|internet|meal|food|travel|commuter|parking|home\s+office|gym(?:\s+membership)?|fitness|child(?:[-\s])?care|dependent\s+care|health(?:care)?|clothing|uniform|WFH|work\s+from\s+home|remote|equipment|laptop|tech(?:nology)?|coworking|transit|mileage|bike|bicycle|cycling|e-?bikes?|caregiver))*"
