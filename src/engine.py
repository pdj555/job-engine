...
_INDEX_PATH_RE = re.compile(
    ...
    r"|/(?:internships|university[-_]?recruiting|campus[-_]?recruiting|early[-_]?careers?|student[-_]?programs?|graduate[-_]?programs?|university[-_]?programs?|life[-_]?at(?:[-_][^/]+)?|team|meet[-_]?(?:the|our)[-_]?team|our[-_]?(?:team|people))/?$"
    r"|/(?:salaries|salary)(?:/|$)"
    r"|/apply/?$",
    re.I,
)
...
_GONE_LISTING_RE = re.compile(
    ...
    r"|(?<!once )(?<!after )(?<!when )(?:the|this)\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening)"
    r"\s+has\s+(?:been\s+)?(?:filled|expired|closed|withdrawn|cancelled|canceled|taken\s+down|ended(?!\s+up)|removed|discontinued|archived)"
    r"|(?<!once )(?<!after )(?<!when )(?<!this )(?<!the )(?<!a )(?:sorry,\s+)?"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening)"
    r"\s+has\s+(?:been\s+)?(?:filled|expired|closed|withdrawn|cancelled|canceled|taken\s+down|ended(?!\s+up)|removed|discontinued|archived)"
    r"|(?:the|this)\s+"
    ...
    r"|we(?:'re| are)\s+no\s+longer\s+(?:hiring\s+for\s+this|accepting\s+applications|taking\s+applications|accepting\s+applicants|taking\s+applicants)"
    r"|no\s+longer\s+(?:accepting|taking)\s+(?:applications|applicants)\b"
