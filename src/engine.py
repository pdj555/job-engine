...
_GONE_LISTING_RE = re.compile(
    ...
    r"|(?<!once )(?<!after )(?<!when )(?:the|this)\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening)"
    r"\s+has\s+(?:been\s+)?(?:filled|expired|closed|withdrawn|cancelled|canceled|taken\s+down|taken\s+offline|ended(?!\s+up)|removed|discontinued|archived|unpublished)"
    r"|(?<!once )(?<!after )(?<!when )(?<!this )(?<!the )(?<!a )(?:sorry,\s+)?"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening)"
    r"\s+has\s+(?:been\s+)?(?:filled|expired|closed|withdrawn|cancelled|canceled|taken\s+down|taken\s+offline|ended(?!\s+up)|removed|discontinued|archived|unpublished)"
    r"|(?:the|this)\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening)"
    r"\s+was\s+(?:withdrawn|cancelled|canceled|removed|taken\s+down|taken\s+offline)"
    r"|we(?:'ve|\s+have)\s+decided\s+not\s+to\s+fill\s+this\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening)"
    r"|(?<!once )(?<!after )(?<!when )(?:the|this)\s+search\s+(?:is|has\s+been)\s+closed\b"
    r"|we\s+have\s+(?:filled|closed|withdrawn)\s+this\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening)"
    r"|(?:the|this)\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening)"
    ...
    r"|no\s+longer\s+(?:accepting|taking)\s+(?:new\s+)?(?:applications|applicants)\b"
    ...
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening)"
    r"|(?:sorry,?\s+)?we\s+couldn['’]t\s+find\s+that\s+"
    ...
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening)"
    r"\s+is\s+no\s+longer\s+open\s+(?:to\s+applicants|for\s+applications)"
    r"|(?:sorry,?\s+)?we\s+couldn['’]t\s+find\s+this\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|listing|opening)\b(?!\s+description)"
    r"|(?:the|this)\s+"
...
_NON_SALARY_MONEY_RE = re.compile(
    ...
    r"\s+(?:in\s+)?(?:equity|stock(?:\s+options?)?|RSUs?|ESPP|employee\s+stock\s+purchase(?:\s+plan)?)\b"
    r"|"
    r"\b(?:equity|stock(?:\s+options?)?|RSUs?|ESPP|employee\s+stock\s+purchase(?:\s+plan)?)\s*(?:of|:)?\s*"
    r"(?:USD|US\$|\$)\s*[\d,]+(?:\.\d+)?(?:\s*k)?"
    r"(?:\s*(?:[-–—]|to)\s*(?:USD|US\$|\$)?\s*[\d,]+(?:\.\d+)?(?:\s*k)?)?"
    r"|"
    r"(?:USD|US\$|\$)\s*[\d,]+(?:\.\d+)?(?:\s*k)?"
    r"(?:\s*(?:[-–—]|to)\s*(?:USD|US\$|\$)?\s*[\d,]+(?:\.\d+)?(?:\s*k)?)?"
    r"\s+(?:(?:employee\s+)?(?:signing|sign-on|sign on|relocation|annual|target|performance|retention|referral|spot|cash|year-end|year end|holiday)\s+bonus|(?:signing|sign-on|sign on)\b)"
    r"|"
    r"\b(?:(?:employee\s+)?(?:signing|sign-on|sign on|relocation|annual|target|performance|retention|referral|spot|cash|year-end|year end|holiday)\s+bonus|(?:signing|sign-on|sign on))\s*(?:of|:)?\s*"
    r"(?:USD|US\$|\$)\s*[\d,]+(?:\.\d+)?(?:\s*k)?"
    r"(?:\s*(?:[-–—]|to)\s*(?:USD|US\$|\$)?\s*[\d,]+(?:\.\d+)?(?:\s*k)?)?"
    r"|"
    r"(?:USD|US\$|\$)\s*[\d,]+(?:\.\d+)?(?:\s*k)?"
    ...
    r"|"
    r"\b(?:(?:employer|annual)\s+)*401\(?k\)?\s+(?:match|contribution)\s*(?:of|:)?\s*"
    r"(?:USD|US\$|\$)\s*[\d,]+(?:\.\d+)?(?:\s*k)?"
    r"(?:\s*(?:[-–—]|to)\s*(?:USD|US\$|\$)?\s*[\d,]+(?:\.\d+)?(?:\s*k)?)?"
    r"|"
    r"(?:USD|US\$|\$)\s*[\d,]+(?:\.\d+)?(?:\s*k)?"
    ...
    r"\s+(?:for\s+)?(?:housing|travel|meals?|food|living|phone|cell|internet)\b"
    r"|"
    r"\b(?:housing|travel|meals?|food|living|phone|cell|internet)\s*(?:of|:)?\s*"
    r"(?:USD|US\$|\$)\s*[\d,]+(?:\.\d+)?(?:\s*k)?"
    r"(?:\s*(?:[-–—]|to)\s*(?:USD|US\$|\$)?\s*[\d,]+(?:\.\d+)?(?:\s*k)?)?"
    r"(?:" + _HOUR_TAIL + r"|" + _DAY_TAIL + r"|" + _WEEK_TAIL + r"|" + _MONTH_TAIL + r")?"
    r"|"
