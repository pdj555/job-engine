...
_GONE_LISTING_RE = re.compile(
    ...
    r"|(?<!once )(?<!after )(?<!when )(?:the|this)\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening|search)"
    r"\s+is\s+no\s+longer\s+being\s+(?:staffed|filled|funded|sourced|hired|budgeted|resourced|allocated|processed|actioned|reviewed|accepted|evaluated|assessed|screened|offered(?!\s+as\b)|pursued(?!\s+as\b)|considered(?!\s+for\b))\b"
    r"|(?<!once )(?<!after )(?<!when )(?:the|this)\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening|search)"
...
def _stated_hours(title: str, description: str) -> Optional[int]:
    """Hours explicitly written as N hours/week, N hours/fortnight or biweekly halved, N hours twice or thrice a fortnight, N hours 2–10 times a week, N hours/day × 5, or 2–7 days at N hours."""
