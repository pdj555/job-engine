...
_GONE_LISTING_RE = re.compile(
    ...
    r"|(?<!once )(?<!after )(?<!when )(?:the|this)\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening|search)"
    r"\s+is\s+no\s+longer\s+being\s+(?:recruited|advertised|posted|listed|published|shared|boosted|sponsored|endorsed|circulated|indexed|featured|promoted|recommended|referred|syndicated|broadcast|marketed|distributed|amplified|surfaced)\b"
    r"|(?<!once )(?<!after )(?<!when )(?:the|this)\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening|search)"
    r"\s+is\s+not\s+being\s+(?:recruited|advertised|filled|posted|listed|published|shared|boosted|sponsored|endorsed|circulated|indexed|featured|promoted|recommended|referred|syndicated|broadcast|marketed|distributed|amplified|surfaced|pursued(?!\s+as\b)|considered(?!\s+for\b))\b"
