...
_GONE_LISTING_RE = re.compile(
    ...
    r"|(?<!once )(?<!after )(?<!when )(?:the|this)\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening|search)"
    r"\s+has\s+(?:already\s+)?(?:been\s+)?(?:filled|expired|lapsed|closed|withdrawn|cancelled|canceled|taken\s+down|taken\s+offline|ended(?!\s+up)|removed|discontinued|archived|unpublished|deactivated|deleted|unposted|pulled|hired\s+internally|rescinded|abandoned|called\s+off|shelved|tabled|sunset|retired|dropped|released|frozen|killed|scrapped|postponed|suspended|deferred|parked|mothballed)"
    r"|(?<!once )(?<!after )(?<!when )(?<!this )(?<!the )(?<!a )(?:sorry,\s+)?"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening)"
    r"\s+has\s+(?:already\s+)?(?:been\s+)?(?:filled|expired|lapsed|closed|withdrawn|cancelled|canceled|taken\s+down|taken\s+offline|ended(?!\s+up)|removed|discontinued|archived|unpublished|deactivated|deleted|unposted|pulled|hired\s+internally|rescinded|abandoned|called\s+off|shelved|tabled|sunset|retired|dropped|released|frozen|killed|scrapped|postponed|suspended|deferred|parked|mothballed)"
    r"|(?<!once )(?<!after )(?<!when )(?:the|this)\s+"
    ...
    r"|we(?:'ve|\s+have)?\s+(?:already\s+)?(?:filled|closed|withdrawn|withdrew|cancelled|canceled|removed|unposted|unpublished|deactivated|archived|deleted|discontinued|pulled|rescinded|abandoned|called\s+off|shelved|tabled|sunset|retired|dropped|released|frozen|killed|scrapped|postponed|suspended|deferred|parked|mothballed)\s+this\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening|search)"
    ...
    r"|(?<!once )(?<!after )(?<!when )(?:the|this)\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening|search)"
    r"\s+(?:is|has\s+been)\s+(?:(?:now|currently)\s+)?(?:closed|expired|lapsed|filled|cancelled|canceled|withdrawn|rescinded|paused|shelved|tabled|sunset|retired|dropped|frozen|killed|scrapped|postponed|suspended|deferred|parked|mothballed)\b(?!\s+with)"
    r"|(?<!once )(?<!after )(?<!when )(?<!this )(?<!the )(?<!a )(?:sorry,\s+)?"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening)"
    r"\s+(?:is|has\s+been)\s+(?:(?:now|currently)\s+)?(?:closed|expired|lapsed|filled|cancelled|canceled|withdrawn|rescinded|paused|shelved|tabled|sunset|retired|dropped|frozen|killed|scrapped|postponed|suspended|deferred|parked|mothballed)\b(?!\s+with)"
    r"|(?<!once )(?<!after )(?<!when )(?:the|this)\s+"
