...
_NON_SALARY_MONEY_RE = re.compile(
    ...
    r"(?:\s*(?:[-–—]|to)\s*(?:USD|US\$|\$)?\s*[\d,]+(?:\.\d+)?(?:\s*k)?)?"
    r"(?:\s*(?:/\s*mo(?:nth)?s?|(?:per|a)\s+mo(?:nth)?s?|monthly))?"
    r"(?:\s+(?:housing|living|relocation|car|vehicle|auto|phone|cell|mobile|internet|meal|food|travel|commuter|parking|home\s+office|gym(?:\s+membership)?|fitness|child(?:[-\s])?care|dependent\s+care|health(?:care)?|clothing|uniform|WFH|work\s+from\s+home|remote|equipment|laptop|tech(?:nology)?|coworking|transit|mileage))*"
    r"\s+allowance\b"
    r"|"
    r"\b(?:(?:monthly|housing|living|relocation|car|vehicle|auto|phone|cell|mobile|internet|meal|food|travel|commuter|parking|clothing|uniform|WFH|work\s+from\s+home|remote|equipment|laptop|tech(?:nology)?|coworking|transit|mileage)\s+)*allowance\s*(?:of|:)?\s*"
    r"(?:USD|US\$|\$)\s*[\d,]+(?:\.\d+)?(?:\s*k)?"
    r"(?:\s*(?:[-–—]|to)\s*(?:USD|US\$|\$)?\s*[\d,]+(?:\.\d+)?(?:\s*k)?)?"
    r"|"
    r"(?:USD|US\$|\$)\s*[\d,]+(?:\.\d+)?(?:\s*k)?"
...
_ONSITE_WORKPLACE_RE = re.compile(
    ...
    r"|(?:office|site|campus)\s+(?:presence|attendance)\s+"
    r"(?:two|three|four|five|\d+(?:\s*[-–]\s*\d+)?)\s+days?\s+(?:(?:a|per)\s+week|weekly)\b"
    r"|(?:two|three|four|five|\d+(?:\s*[-–]\s*\d+)?)\s+days?\s+(?:of\s+)?(?:office|site|campus)\s+(?:presence|attendance)"
