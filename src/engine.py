...
_GONE_LISTING_RE = re.compile(
    ...
    r"|we(?:'ve|\s+have)?\s+(?:already\s+)?(?:filled|closed|withdrawn|withdrew|cancelled|canceled|removed|unposted|unpublished|deactivated|archived|deleted|discontinued|pulled|rescinded|abandoned|called\s+off|shelved|tabled|sunset|retired|dropped|released|frozen|killed|scrapped|postponed|suspended|deferred|parked|mothballed|hibernated|stalled|halted|idled|inactivated|iced|shuttered|sidelined|canned|axed|retracted|voided|revoked|recalled|boxed|folded|unlisted|junked|ditched|dumped|tossed|chucked|scrubbed|spiked|bagged|binned|trashed|shredded|pulped|nuked|discarded|purged|yanked|wiped|expunged|jettisoned|redacted|annulled|nullified|vacated|invalidated|quashed|scuttled|torpedoed|obliterated|erased|vaporized|pulverized|pulverised|atomized|annihilated|extinguished|extirpated|excised|abrogated|repealed|overruled|overturned|dissolved|disbanded|liquidated|aborted|deep[-\s]sixed|crushed|flattened|buried|gutted|whacked|snuffed|sunsetted|decommissioned|dismantled|demolished|wrecked|de[-\s]?funded|eliminated|abolished|disestablished|un[-\s]?funded|superseded|reallocated|zero[-\s]?funded|backfilled|de[-\s]?staffed|un[-\s]?staffed|un[-\s]?resourced|de[-\s]?resourced|un[-\s]?allocated|de[-\s]?allocated|un[-\s]?budgeted|de[-\s]?budgeted)\s+this\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening|search)"
...
_HOURS_RE = re.compile(
    ...
    r"(?!\s+(?:meeting|standup|stand-up|sync|call|all-?hands))"
    r"|(?:hours?|hrs?)\s*(?:worked\s*)?[:=\-–—]\s*(\d{1,2}(?:\.\d+)?)\+?\s*(?:p(?:er[\s.]*)?\.?\s*/?|/|an?\.?\s*/?\s*)(?:wk|weeks?|weekly)\b"
    r"|work[\s-]*weeks?\s*[:=\-–—]\s*(\d{1,2}(?:\.\d+)?)\+?\s*(?:hours?|hrs?)?\b"
    r"|(?:hours?|hrs?)\s+of\s+(?:the\s+)?(?:work(?:ing)?|scheduled)\s*(?:(?:p(?:er[\s.]*)?\.?\s*/?|/|an?\.?\s*/?)\s*)?(?:wk|weeks?|weekly)\s*[:=\-–—]?\s*(\d{1,2}(?:\.\d+)?)\+?"
    r"|(?<![\d.])(\d{1,2}(?:\.\d+)?)\+?[\s-]*hpw\b"
...
_FTE_FRAC_RE = re.compile(
    ...
    r"|(?<![\d.])\b((?:one[-\s])?twenty[-\s]first)[-\s]time\s*fte\b"
    r"|\bfte\s*[:=\-–—]?\s*((?:one[-\s])?twenty[-\s]first)[-\s]time\b"
