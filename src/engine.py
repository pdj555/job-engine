...
_NON_SALARY_MONEY_RE = re.compile(
    ...
    r"(?:\s*(?:[-–—]|to)\s*(?:USD|US\$|\$)?\s*[\d,]+(?:\s+\d{3})*(?:\.\d+)?(?:\s*k)?)?"
    r"(?:" + _ANY_PERIOD + r")?"
    r"\s+(?:GTL|(?:group(?:\s+term)?|supplemental|basic|voluntary|term)\s+life(?:\s+insurance)?|split[-\s]dollar(?:\s+(?:life|plan|insurance))?|key(?:[-\s]?person|[-\s]man)\s+life|(?:corporate|bank|company|employer)[-\s]owned\s+life)(?:\s+(?:benefit|coverage|offset|rebate|max|cap|insurance))?+(?:\s+(?:clawback|recoupment|recovery|repayment|holdback))?+\b(?!\s+in)(?!\s*(?:of|:)?\s*(?:USD|US\$|\$)\s*[\d,])"
    r"|"
    r"\b(?:GTL|(?:group(?:\s+term)?|supplemental|basic|voluntary|term)\s+life(?:\s+insurance)?|split[-\s]dollar(?:\s+(?:life|plan|insurance))?|key(?:[-\s]?person|[-\s]man)\s+life|(?:corporate|bank|company|employer)[-\s]owned\s+life)\s*(?:of|:)?\s*"
    r"(?:USD|US\$|\$)\s*[\d,]+(?:\s+\d{3})*(?:\.\d+)?(?:\s*k)?"
    r"(?:\s*(?:[-–—]|to)\s*(?:USD|US\$|\$)?\s*[\d,]+(?:\s+\d{3})*(?:\.\d+)?(?:\s*k)?)?"
    r"|"
    r"(?:USD|US\$|\$)\s*[\d,]+(?:\s+\d{3})*(?:\.\d+)?(?:\s*k)?"
