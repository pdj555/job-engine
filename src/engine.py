...
_INDEX_PATH_RE = re.compile(
    ...
    r"|/(?:internships|university[-_]?recruiting|campus[-_]?recruiting|early[-_]?careers?|student[-_]?programs?|graduate[-_]?programs?|university[-_]?programs?|job[-_]?search|life[-_]?at(?:[-_][^/]+)?|team|meet[-_]?(?:the|our)[-_]?team|our[-_]?(?:team|people)|benefits|our[-_]?benefits|culture|our[-_]?culture|leadership|our[-_]?leadership)/?$"
    r"|/(?:salaries|salary)(?:/|$)"
    r"|/apply/?$",
    re.I,
)
...
_GONE_LISTING_RE = re.compile(
    ...
    r"\s+is\s+no\s+longer\s+(?:available|active|posted|listed|live)"
    ...
    r"|(?<!once )(?<!after )(?<!when )(?:the|this)\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening)"
    r"\s+has\s+(?:been\s+)?(?:filled|expired|closed|withdrawn|cancelled|canceled|taken\s+down|ended(?!\s+up)|removed|discontinued|archived|unpublished)"
    r"|(?<!once )(?<!after )(?<!when )(?<!this )(?<!the )(?<!a )(?:sorry,\s+)?"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening)"
    r"\s+has\s+(?:been\s+)?(?:filled|expired|closed|withdrawn|cancelled|canceled|taken\s+down|ended(?!\s+up)|removed|discontinued|archived|unpublished)"
    r"|(?:the|this)\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening)"
    r"\s+was\s+(?:withdrawn|cancelled|canceled|removed)"
    r"|we\s+have\s+(?:filled|closed|withdrawn)\s+this\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening)"
    r"|(?:the|this)\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening)"
    r"\s+(?:is|has\s+been)\s+(?:closed|expired)"
    r"|we(?:'re| are)\s+no\s+longer\s+(?:hiring\s+for\s+this|accepting\s+(?:new\s+)?(?:applications|applicants)|taking\s+(?:new\s+)?(?:applications|applicants))"
    ...
    r"(?:available|active|posted|listed|live)\b"
)
...
_NON_SALARY_MONEY_RE = re.compile(
    ...
    r"\s+(?:in\s+)?(?:equity|stock(?:\s+options?)?|RSUs?)\b"
    r"|"
    ...
    r"(?:USD|US\$|\$)\s*[\d,]+(?:\.\d+)?(?:\s*k)?"
    r"(?:\s*(?:[-–—]|to)\s*(?:USD|US\$|\$)?\s*[\d,]+(?:\.\d+)?(?:\s*k)?)?"
    r"\s+(?:(?:employee\s+)?(?:signing|sign-on|sign on|relocation|annual|target|performance|retention|referral|spot)\s+bonus|(?:signing|sign-on|sign on)\b)"
    r"|"
    r"\b(?:(?:employee\s+)?(?:signing|sign-on|sign on|relocation|annual|target|performance|retention|referral|spot)\s+bonus|(?:signing|sign-on|sign on))\s*(?:of|:)?\s*"
    r"(?:USD|US\$|\$)\s*[\d,]+(?:\.\d+)?(?:\s*k)?"
    r"(?:\s*(?:[-–—]|to)\s*(?:USD|US\$|\$)?\s*[\d,]+(?:\.\d+)?(?:\s*k)?)?"
    r"|"
    r"(?:USD|US\$|\$)\s*[\d,]+(?:\.\d+)?(?:\s*k)?"
    ...
    r"(?:USD|US\$|\$)\s*[\d,]+(?:\.\d+)?(?:\s*k)?"
    r"(?:\s*(?:[-–—]|to)\s*(?:USD|US\$|\$)?\s*[\d,]+(?:\.\d+)?(?:\s*k)?)?"
    r"\s+(?:tuition(?:\s+(?:reimbursement|assistance|benefit))?(?!\s+in)|education\s+(?:reimbursement|assistance|benefit|budget)|student[-\s]loans?(?:\s+repayment)?|professional\s+development(?:\s+(?:budget|reimbursement|allowance|benefit))?|learning(?:\s+and\s+development(?:\s+(?:budget|reimbursement|allowance|benefit))?|\s+(?:budget|reimbursement|allowance|benefit))|continuing\s+education(?:\s+(?:budget|reimbursement|allowance|benefit))?|conference\s+(?:budget|reimbursement|allowance|benefit)|training\s+(?:budget|reimbursement|allowance|benefit)|wellness\s+(?:budget|reimbursement|allowance|benefit|program)|fertility\s+(?:benefit|coverage|assistance|budget)|parental\s+leave|child(?:[-\s])?care\s+(?:benefit|stipend|allowance|assistance|budget)|dependent\s+care\s+(?:benefit|stipend|allowance|assistance|budget)|(?:fitness|gym(?:\s+membership)?|commuter|parking|phone|cell|internet|home\s+office)\s+reimbursement)\b"
    r"|"
    r"\b(?:tuition(?:\s+(?:reimbursement|assistance|benefit))?|education\s+(?:reimbursement|assistance|benefit|budget)|student[-\s]loans?(?:\s+repayment)?|professional\s+development(?:\s+(?:budget|reimbursement|allowance|benefit))?|learning(?:\s+and\s+development(?:\s+(?:budget|reimbursement|allowance|benefit))?|\s+(?:budget|reimbursement|allowance|benefit))|continuing\s+education(?:\s+(?:budget|reimbursement|allowance|benefit))?|conference\s+(?:budget|reimbursement|allowance|benefit)|training\s+(?:budget|reimbursement|allowance|benefit)|wellness\s+(?:budget|reimbursement|allowance|benefit|program)|fertility\s+(?:benefit|coverage|assistance|budget)|parental\s+leave|child(?:[-\s])?care\s+(?:benefit|stipend|allowance|assistance|budget)|dependent\s+care\s+(?:benefit|stipend|allowance|assistance|budget)|(?:fitness|gym(?:\s+membership)?|commuter|parking|phone|cell|internet|home\s+office)\s+reimbursement)\s*(?:of|:)?\s*"
    r"(?:USD|US\$|\$)\s*[\d,]+(?:\.\d+)?(?:\s*k)?"
    r"(?:\s*(?:[-–—]|to)\s*(?:USD|US\$|\$)?\s*[\d,]+(?:\.\d+)?(?:\s*k)?)?"
    r"|"
    r"(?:USD|US\$|\$)\s*[\d,]+(?:\.\d+)?(?:\s*k)?"
    ...
    r"(?:\s+(?:housing|living|relocation|meal|food|travel|wellness|phone|cell|internet|commuter|parking|home\s+office|gym(?:\s+membership)?|fitness|child(?:[-\s])?care|dependent\s+care))*"
    r"\s+stipend\b"
    r"|"
    ...
    r"(?:\s*(?:[-–—]|to)\s*(?:USD|US\$|\$)?\s*[\d,]+(?:\.\d+)?(?:\s*k)?)?"
    r"(?:\s*(?:/\s*mo(?:nth)?s?|(?:per|a)\s+mo(?:nth)?s?|monthly))?"
    r"(?:\s+(?:housing|living|relocation|car|vehicle|auto|phone|cell|internet|meal|food|travel|commuter|parking|home\s+office|gym(?:\s+membership)?|fitness|child(?:[-\s])?care|dependent\s+care))*"
    r"\s+allowance\b"
    r"|"
    ...
    r"\s+(?:for\s+)?(?:housing|travel|meals?|food|living)\b"
    r"|"
    r"\b(?:housing|travel|meals?|food|living)\s*(?:of|:)?\s*"
    r"(?:USD|US\$|\$)\s*[\d,]+(?:\.\d+)?(?:\s*k)?"
    r"(?:\s*(?:[-–—]|to)\s*(?:USD|US\$|\$)?\s*[\d,]+(?:\.\d+)?(?:\s*k)?)?"
    r"(?:" + _HOUR_TAIL + r"|" + _DAY_TAIL + r"|" + _WEEK_TAIL + r"|" + _MONTH_TAIL + r")?"
    r"|"
