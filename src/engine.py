...
_GONE_LISTING_RE = re.compile(
    ...
    r"|(?<!once )(?<!after )(?<!when )(?:the|this)\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening|search)"
    r"\s+is\s+no\s+longer\s+being\s+(?:staffed|filled|funded|sourced|hired|budgeted|resourced|allocated|processed|actioned|reviewed|accepted|evaluated|assessed|screened|interviewed|shortlisted|scheduled|selected|ranked|assigned|matched|offered(?!\s+as\b)|pursued(?!\s+as\b)|considered(?!\s+for\b))\b"
    r"|(?<!once )(?<!after )(?<!when )(?:the|this)\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening|search)"
