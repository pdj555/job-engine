...
_NON_SALARY_MONEY_RE = re.compile(
    ...
    r"(?:\s*(?:[-–—]|to)\s*(?:USD|US\$|\$)?\s*[\d,]+(?:\s+\d{3})*(?:\.\d+)?(?:\s*k)?)?"
    r"(?:" + _ANY_PERIOD + r")?"
    r"\s+(?:Citi[-\s]?bikes?|Blue[-\s]?bikes?|Divvy[-\s]?bikes?|Lime[-\s]?bikes?|Jump[-\s]?bikes?|Bay[-\s]?Wheels|(?:(?!in\b)\S+\s+)?bike[-\s]?share)\b(?!\s+in)(?!\s*(?:of|:)?\s*(?:USD|US\$|\$)\s*[\d,])"
    r"|"
    r"\b(?:Citi[-\s]?bikes?|Blue[-\s]?bikes?|Divvy[-\s]?bikes?|Lime[-\s]?bikes?|Jump[-\s]?bikes?|Bay[-\s]?Wheels|(?:(?!in\b)\S+\s+)?bike[-\s]?share)\s*(?:of|:)?\s*"
    r"(?:USD|US\$|\$)\s*[\d,]+(?:\s+\d{3})*(?:\.\d+)?(?:\s*k)?"
    r"(?:\s*(?:[-–—]|to)\s*(?:USD|US\$|\$)?\s*[\d,]+(?:\s+\d{3})*(?:\.\d+)?(?:\s*k)?)?"
    r"|"
    r"(?:USD|US\$|\$)\s*[\d,]+(?:\s+\d{3})*(?:\.\d+)?(?:\s*k)?"
