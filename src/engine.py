...
_HOURS_RE = re.compile(
    ...
    r"|(?:hours?|hrs?)\s+(?:of\s+(?:the\s+)?)?(?:work(?:ing)?|scheduled)\s*(?:(?:per(?:\s+(?:the|each|every))|p(?:er[\s.]*)?\.?\s*/?|/\s*(?:(?:the|each|every)\s+)?|an?\.?\s*/?|for(?:\s+(?:the|each|every))?|each|every)\s*)?(?:weekly|wkly|weeks?|wk|w)\.?\s*[:=\-–—]?\s*(\d{1,2}(?:\.\d+)?)\+?"
    r"|(?<![\d.])(\d{1,2}(?:\.\d+)?)\+?[\s-]*hpw\b"
