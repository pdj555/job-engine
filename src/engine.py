...
_ONSITE_WORKPLACE_RE = re.compile(
    ...
    r"|\b(?:travel|drive|go|head|fly|walk|ride|return|bike|cycle|scooter|motorcycle|motorbike|moped|boat|ferry|kayak|canoe|paddle|row|skateboard|skate)\s+(?:onto|to)\s+(?:the\s+|our\s+|an\s+)?(?:\S+\s+){0,4}(?<!home )(?<!home-)(?:offices?|campus|lab(?:oratory)?|headquarters|hq|hub|(?<!off-)(?<!off )site|field(?!\s+of\b))\b(?!\s+hours)"
