...
_GONE_LISTING_RE = re.compile(
    ...
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening)"
    r"\s+has\s+(?:been\s+)?(?:filled|expired|closed|withdrawn|cancelled|canceled|taken\s+down)"
...
_NON_SALARY_MONEY_RE = re.compile(
    ...
    r"(?:USD|US\$|\$)\s*[\d,]+(?:\.\d+)?(?:\s*k)?"
    r"(?:\s*(?:[-–—]|to)\s*(?:USD|US\$|\$)?\s*[\d,]+(?:\.\d+)?(?:\s*k)?)?"
    r"\s+(?:tuition(?:\s+(?:reimbursement|assistance|benefit))?(?!\s+in)|education\s+(?:reimbursement|assistance|benefit|budget)|student[-\s]loans?(?:\s+repayment)?|professional\s+development(?:\s+(?:budget|reimbursement|allowance|benefit))?|learning(?:\s+and\s+development(?:\s+(?:budget|reimbursement|allowance|benefit))?|\s+(?:budget|reimbursement|allowance|benefit))|continuing\s+education(?:\s+(?:budget|reimbursement|allowance|benefit))?)\b"
    r"|"
    r"\b(?:tuition(?:\s+(?:reimbursement|assistance|benefit))?|education\s+(?:reimbursement|assistance|benefit|budget)|student[-\s]loans?(?:\s+repayment)?|professional\s+development(?:\s+(?:budget|reimbursement|allowance|benefit))?|learning(?:\s+and\s+development(?:\s+(?:budget|reimbursement|allowance|benefit))?|\s+(?:budget|reimbursement|allowance|benefit))|continuing\s+education(?:\s+(?:budget|reimbursement|allowance|benefit))?)\s*(?:of|:)?\s*"
    r"(?:USD|US\$|\$)\s*[\d,]+(?:\.\d+)?(?:\s*k)?"
    r"(?:\s*(?:[-–—]|to)\s*(?:USD|US\$|\$)?\s*[\d,]+(?:\.\d+)?(?:\s*k)?)?"
    r"|"
    r"(?:USD|US\$|\$)\s*[\d,]+(?:\.\d+)?(?:\s*k)?"
    ...
    r"(?:\s+(?:housing|living|relocation|meal|food|travel|wellness|phone|cell|internet|commuter|parking|home\s+office))*"
    r"\s+stipend\b"
    r"|"
    ...
    r"(?:\s*(?:[-–—]|to)\s*(?:USD|US\$|\$)?\s*[\d,]+(?:\.\d+)?(?:\s*k)?)?"
    r"(?:\s*(?:/\s*mo(?:nth)?s?|(?:per|a)\s+mo(?:nth)?s?|monthly))?"
    r"(?:\s+(?:housing|living|relocation|car|vehicle|auto|phone|cell|internet|meal|food|travel|commuter|parking|home\s+office))*"
    r"\s+allowance\b"
    r"|"
