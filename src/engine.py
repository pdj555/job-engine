...
_GONE_LISTING_RE = re.compile(
    ...
    r"|(?<!once )(?<!after )(?<!when )(?:the|this)\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening|search)"
    r"\s+has\s+(?:already\s+)?(?:been\s+)?(?:filled|expired|closed|withdrawn|cancelled|canceled|taken\s+down|taken\s+offline|ended(?!\s+up)|removed|discontinued|archived|unpublished|deactivated|deleted|unposted|pulled|hired\s+internally|rescinded)"
    r"|(?<!once )(?<!after )(?<!when )(?<!this )(?<!the )(?<!a )(?:sorry,\s+)?"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening)"
    r"\s+has\s+(?:already\s+)?(?:been\s+)?(?:filled|expired|closed|withdrawn|cancelled|canceled|taken\s+down|taken\s+offline|ended(?!\s+up)|removed|discontinued|archived|unpublished|deactivated|deleted|unposted|pulled|hired\s+internally|rescinded)"
    r"|(?<!once )(?<!after )(?<!when )(?:the|this)\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening|search)"
    r"\s+was\s+(?:already\s+)?(?:filled|expired|closed|withdrawn|cancelled|canceled|taken\s+down|taken\s+offline|ended(?!\s+up)|removed|discontinued|archived|unpublished|deactivated|deleted|unposted|pulled|hired\s+internally|rescinded)"
    r"|(?<!once )(?<!after )(?<!when )(?<!this )(?<!the )(?<!a )(?:sorry,\s+)?"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening)"
    r"\s+was\s+(?:already\s+)?(?:filled|expired|closed|withdrawn|cancelled|canceled|taken\s+down|taken\s+offline|ended(?!\s+up)|removed|discontinued|archived|unpublished|deactivated|deleted|unposted|pulled|hired\s+internally|rescinded)"
    r"|we(?:'ve|\s+have)?\s+(?:decided|chosen|chose|opted|elected)\s+(?:not\s+to\s+(?:fill|(?:proceed|move\s+forward|continue)\s+with|hire\s+for|pursue)|against(?:\s+(?:filling|(?:proceeding|moving\s+forward|continuing)\s+with|hiring\s+for|pursuing))?)\s+this\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening|search)"
    r"|we(?:'ve|\s+have)?\s+moved\s+on\s+from\s+this\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening|search)"
    r"|(?<!once )(?<!after )(?<!when )(?:the|this)\s+search\s+(?:is|has\s+been|was|has)\s+(?:closed|filled|expired|withdrawn|removed|taken\s+down|taken\s+offline|unpublished|archived|deactivated|deleted|unposted|pulled|discontinued)\b"
    ...
    r"|we(?:'ve|\s+have)?\s+(?:already\s+)?(?:filled|closed|withdrawn|withdrew|cancelled|canceled|removed|unposted|unpublished|deactivated|archived|deleted|discontinued|pulled|rescinded)\s+this\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening|search)"
    ...
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening|search)"
    r"|(?<!once )(?<!after )(?<!when )(?:a|the)\s+hire\s+has\s+been\s+made\s+for\s+this\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening|search)"
    r"|we(?:'ve|\s+have)?\s+(?:selected|chosen)\s+(?:(?:a|an|the|another|other)\s+)?(?:candidate|applicant)s?\s+for\s+this\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening|search)"
    r"|(?<!once )(?<!after )(?<!when )(?:a|an|the)\s+(?:candidate|applicant)\s+has\s+been\s+(?:selected|chosen|hired)\s+(?:internally\s+)?for\s+this\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening|search)"
    r"|(?<!once )(?<!after )(?<!when )someone\s+has\s+been\s+hired\s+(?:internally\s+)?for\s+this\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening|search)"
    ...
    r"|(?<!once )(?<!after )(?<!when )(?:the|this)\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening|search)"
    r"\s+(?:is|has\s+been)\s+(?:(?:now|currently)\s+)?(?:closed|expired|filled|cancelled|canceled|withdrawn|rescinded|paused)\b(?!\s+with)"
    r"|(?<!once )(?<!after )(?<!when )(?<!this )(?<!the )(?<!a )(?:sorry,\s+)?"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening)"
    r"\s+(?:is|has\s+been)\s+(?:(?:now|currently)\s+)?(?:closed|expired|filled|cancelled|canceled|withdrawn|rescinded|paused)\b(?!\s+with)"
    r"|(?<!once )(?<!after )(?<!when )(?:the|this)\s+"
    ...
    r"|(?<!once )(?<!after )(?<!when )(?:the|this)\s+search\s+(?:is|has\s+been|was|has)\s+(?:cancelled|canceled)\b"
    r"|(?<!once )(?<!after )(?<!when )(?:the|this)\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening|search)"
    r"\s+is\s+no\s+longer\s+being\s+(?:recruited|advertised|posted|listed|published)\b"
    r"|(?<!once )(?<!after )(?<!when )(?:the|this)\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening|search)"
    r"\s+is\s+not\s+being\s+(?:recruited|advertised|filled|posted|listed|published)\b"
    ...
    r"|we\s+will\s+not\s+be\s+(?:filling|(?:proceeding|moving\s+forward|continuing)\s+with|pursuing)\s+this\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening|search)"
    r"|we\s+will\s+not\s+(?:fill|(?:proceed|move\s+forward|continue)\s+with|pursue)\s+this\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening|search)"
    r"|(?<!once )(?<!after )(?<!when )(?:the|this)\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening|search)"
    ...
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening|search)"
    r"|we(?:'re| are)\s+no\s+longer\s+(?:filling|(?:proceeding|moving\s+forward|continuing)\s+with|pursuing)\s+this\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening|search)"
    r"|we(?:(?:'re| are)\s+not| aren't)\s+(?:(?:filling|(?:proceeding|moving\s+forward|continuing)\s+with|pursuing)|going\s+to\s+(?:fill|(?:proceed|move\s+forward|continue)\s+with|hire\s+for|pursue))\s+this\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening|search)"
    r"|(?<!once )(?<!after )(?<!when )(?:the|this)\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening|search)"
    ...
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening|search)"
    r"|we(?:'ve|\s+have)?\s+(?:cancelled|canceled|discontinued)\s+(?:hiring|recruiting)\s+for\s+this\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening|search)"
    r"|(?:hiring|recruiting)\s+has\s+been\s+(?:cancelled|canceled|discontinued)\s+for\s+this\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening|search)"
    ...
    r"|(?<!once )(?<!after )(?<!when )(?:the|this)\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening|search)"
    r"\s+is\s+no\s+longer\s+open\s+(?:to|for)\s+(?:applicants|applications|candidates)"
    r"|(?<!once )(?<!after )(?<!when )(?<!this )(?<!the )(?<!a )"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening)"
    r"\s+is\s+no\s+longer\s+open\s+(?:to|for)\s+(?:applicants|applications|candidates)"
