...
_RELATED_HEADING_RE = re.compile(
    ...
    r"|jobs\s+at\s+\S+"
    r"|all\s+jobs"
    r"|see\s+all\s+(?:jobs|openings)"
    r"|discover\s+openings"
    r"|jobs\s+you\s+applied\s+to"
    r"|explore\s+openings"
...
_GONE_LISTING_RE = re.compile(
    ...
    r"|(?<!once )(?<!after )(?<!when )(?:the|this)\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening)"
    r"\s+has\s+(?:been\s+)?(?:filled|expired|closed|withdrawn|cancelled|canceled|taken\s+down|taken\s+offline|ended(?!\s+up)|removed|discontinued|archived|unpublished|deactivated|deleted|unposted)"
    r"|(?<!once )(?<!after )(?<!when )(?<!this )(?<!the )(?<!a )(?:sorry,\s+)?"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening)"
    r"\s+has\s+(?:been\s+)?(?:filled|expired|closed|withdrawn|cancelled|canceled|taken\s+down|taken\s+offline|ended(?!\s+up)|removed|discontinued|archived|unpublished|deactivated|deleted|unposted)"
    r"|(?:the|this)\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening)"
    r"\s+was\s+(?:withdrawn|cancelled|canceled|removed|taken\s+down|taken\s+offline|deactivated|unpublished)"
    r"|we(?:'ve|\s+have)\s+decided\s+not\s+to\s+fill\s+this\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening)"
    r"|(?<!once )(?<!after )(?<!when )(?:the|this)\s+search\s+(?:is|has\s+been)\s+closed\b"
    ...
    r"|we(?:'ve|\s+have)\s+(?:filled|closed|withdrawn|cancelled|canceled|removed|unposted|unpublished)\s+this\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening)"
    r"|(?:the|this)\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening)"
    ...
    r"|(?<!once )(?<!after )(?<!when )(?:the|this)\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening)"
    r"\s+is\s+(?:unpublished|archived)\b"
    r"|(?<!once )(?<!after )(?<!when )(?<!this )(?<!the )(?<!a )(?:sorry,\s+)?"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening)"
    r"\s+is\s+(?:unpublished|archived)\b"
    r"|we(?:'re| are)\s+no\s+longer\s+(?:hiring\s+for\s+this|recruiting\s+for\s+this|accepting\s+(?:new\s+)?(?:applications|applicants)|taking\s+(?:new\s+)?(?:applications|applicants))"
    r"|we(?:'re| are)\s+no\s+longer\s+recruiting\s+this\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening)"
    r"|we(?:'ve|\s+have)\s+closed\s+recruiting\s+for\s+this\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening)"
    r"|we(?:'ve|\s+have)\s+stopped\s+accepting\s+(?:new\s+)?(?:applications|applicants)\b(?!\s+from)"
    ...
    r"|(?:the|this)\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening)"
    r"\s+is\s+no\s+longer\s+being\s+recruited\b"
    r"|we(?:'re| are)\s+not\s+accepting\s+applications\s+for\s+this\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening)"
    r"|no\s+longer\s+(?:accepting|taking)\s+(?:new\s+)?(?:applications|applicants|candidates)\b"
    ...
    r"|we\s+will\s+not\s+be\s+filling\s+this\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening)"
    r"|we(?:'re| are)\s+no\s+longer\s+filling\s+this\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening)"
    r"|(?:the|this)\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening)"
    r"\s+is\s+no\s+longer\s+being\s+filled\b"
    r"|applications\s+closed\s+for\s+this\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening)"
    r"|applications\s+for\s+(?:the|this)\s+search\s+(?:are|is|have|has)\s+(?:now\s+)?closed"
    r"|we(?:'ve|\s+have)\s+paused\s+hiring\s+for\s+this\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening)"
    r"|hiring\s+for\s+this\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening)"
    r"\s+(?:is\s+paused|has\s+ended)\b"
    r"|we(?:'ve|\s+have)\s+put\s+this\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening)"
    r"\s+on\s+hold\b"
    r"|(?<!once )(?<!after )(?<!when )(?:the|this)\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening)"
    r"\s+(?:has\s+been\s+|is\s+)?(?:put\s+)?on\s+hold\b"
    r"|we\s+are\s+not\s+currently\s+hiring\s+for\s+this\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening)"
    ...
    r"|(?:the\s+)?application\s+(?:window|period)\s+(?:has\s+closed|is\s+closed|has\s+ended)"
    r"|(?<!once )(?<!after )(?<!when )applications\s+are\s+(?:now\s+)?closed\b"
    r"|(?<!once )(?<!after )(?<!when )(?:the|this)\s+application\s+(?:is|has\s+been)\s+closed\b"
    r"|(?:the\s+)?application\s+deadline\s+has\s+(?:passed|expired)\b"
    r"|(?:the|this)\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening)"
    ...
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening)"
    r"\s+is\s+no\s+longer\s+open\s+(?:to\s+applicants|for\s+applications)"
    r"|(?:sorry,?\s+)?we\s+could(?:n['’]t|\s+not)\s+locate\s+this\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|listing|opening)\b(?!\s+description)"
    r"|(?:sorry,?\s+)?we\s+could(?:n['’]t|\s+not)\s+find\s+this\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|listing|opening)\b(?!\s+description)"
    r"|(?:sorry,?\s+)?we\s+could(?:n['’]t|\s+not)\s+find\s+that\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|listing|opening)\b(?!\s+description)"
    r"|(?:the|this)\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|listing|opening)"
    r"\s+(?:could\s+not|cannot)\s+be\s+found\b"
    r"|the\s+(?:job(?:\s+posting)?|role|position|posting|listing)"
    r"\s+you\s+(?:are|were)\s+looking\s+for\s+is\s+no\s+longer\s+"
    r"(?:available|active|posted|listed|live|published)\b"
    r"|the\s+(?:job(?:\s+posting)?|role|position|posting|listing)"
    r"\s+you\s+(?:requested|selected)\s+no\s+longer\s+exists\b"
    r"|the\s+(?:job(?:\s+posting)?|role|position|posting|listing)"
    r"\s+you\s+(?:requested|selected)\s+could\s+not\s+be\s+found\b"
    r"|(?<!once )(?<!after )(?<!when )(?:the|this)\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening)"
...
_NON_SALARY_MONEY_RE = re.compile(
    ...
    r"\s+(?:in\s+)?(?:equity|stock(?:\s+options?)?|RSUs?|ESPP|employee\s+stock(?:\s+purchase(?:\s+plan)?)?|restricted\s+stock|severance)\b"
    r"|"
    r"\b(?:equity|stock(?:\s+options?)?|RSUs?|ESPP|employee\s+stock(?:\s+purchase(?:\s+plan)?)?|restricted\s+stock|severance)\s*(?:of|:)?\s*"
    r"(?:USD|US\$|\$)\s*[\d,]+(?:\.\d+)?(?:\s*k)?"
    r"(?:\s*(?:[-–—]|to)\s*(?:USD|US\$|\$)?\s*[\d,]+(?:\.\d+)?(?:\s*k)?)?"
    r"|"
    r"(?:USD|US\$|\$)\s*[\d,]+(?:\.\d+)?(?:\s*k)?"
    r"(?:\s*(?:[-–—]|to)\s*(?:USD|US\$|\$)?\s*[\d,]+(?:\.\d+)?(?:\s*k)?)?"
    r"\s+(?:(?:employee\s+)?(?:signing|sign-on|sign on|signon|relocation|annual|target|performance|retention|referral|spot|cash|year-end|year end|holiday|discretionary|quarterly|incentive|sales|monthly|stay|completion|anniversary|hiring|welcome|joining|new[-\s]hire|baby|new[-\s]parent|peer|patent|tenure|christmas|moving)\s+bonus|(?:signing|sign-on|sign on|signon)\b|(?:employee\s+)?referral\s+award|employee\s+referral\b|(?:spot|recognition)\s+award|holiday\s+gift)"
    r"|"
    r"\b(?:(?:employee\s+)?(?:signing|sign-on|sign on|signon|relocation|annual|target|performance|retention|referral|spot|cash|year-end|year end|holiday|discretionary|quarterly|incentive|sales|monthly|stay|completion|anniversary|hiring|welcome|joining|new[-\s]hire|baby|new[-\s]parent|peer|patent|tenure|christmas|moving)\s+bonus|(?:signing|sign-on|sign on|signon)|(?:employee\s+)?referral\s+award|employee\s+referral|(?:spot|recognition)\s+award|holiday\s+gift)\s*(?:of|:)?\s*"
    r"(?:USD|US\$|\$)\s*[\d,]+(?:\.\d+)?(?:\s*k)?"
    r"(?:\s*(?:[-–—]|to)\s*(?:USD|US\$|\$)?\s*[\d,]+(?:\.\d+)?(?:\s*k)?)?"
    r"|"
    r"(?:USD|US\$|\$)\s*[\d,]+(?:\.\d+)?(?:\s*k)?"
    ...
    r"(?:USD|US\$|\$)\s*[\d,]+(?:\.\d+)?(?:\s*k)?"
    r"(?:\s*(?:[-–—]|to)\s*(?:USD|US\$|\$)?\s*[\d,]+(?:\.\d+)?(?:\s*k)?)?"
    r"\s+(?:OTE\b|on[- ]target\s+earnings\b|commission\b|TC\b|total\s+comp(?:ensation)?\b|incentive\s+compensation\b|variable\s+(?:pay|compensation)\b|deferred\s+compensation\b)"
    r"|"
    r"\b(?:OTE|on[- ]target\s+earnings|commission|TC|total\s+comp(?:ensation)?|incentive\s+compensation|variable\s+(?:pay|compensation)|deferred\s+compensation)\s*(?:of|:)?\s*"
    r"(?:USD|US\$|\$)\s*[\d,]+(?:\.\d+)?(?:\s*k)?"
    r"(?:\s*(?:[-–—]|to)\s*(?:USD|US\$|\$)?\s*[\d,]+(?:\.\d+)?(?:\s*k)?)?"
    r"|"
    r"(?:USD|US\$|\$)\s*[\d,]+(?:\.\d+)?(?:\s*k)?"
    ...
    r"(?:USD|US\$|\$)\s*[\d,]+(?:\.\d+)?(?:\s*k)?"
    r"(?:\s*(?:[-–—]|to)\s*(?:USD|US\$|\$)?\s*[\d,]+(?:\.\d+)?(?:\s*k)?)?"
    r"\s+(?:matching\s+gift|charitable\s+match|COBRA\s+subsidy)\b"
    r"|"
    r"\b(?:matching\s+gift|charitable\s+match|COBRA\s+subsidy)\s*(?:of|:)?\s*"
    r"(?:USD|US\$|\$)\s*[\d,]+(?:\.\d+)?(?:\s*k)?"
    r"(?:\s*(?:[-–—]|to)\s*(?:USD|US\$|\$)?\s*[\d,]+(?:\.\d+)?(?:\s*k)?)?"
    r"|"
    r"(?:USD|US\$|\$)\s*[\d,]+(?:\.\d+)?(?:\s*k)?"
    r"(?:\s*(?:[-–—]|to)\s*(?:USD|US\$|\$)?\s*[\d,]+(?:\.\d+)?(?:\s*k)?)?"
    r"\s+(?:HSA|FSA|HRA|QSEHRA|ICHRA|pension|529|health\s+savings\s+account|health\s+reimbursement\s+arrangement)\s+(?:contribution|benefit)\b"
    r"|"
    r"\b(?:HSA|FSA|HRA|QSEHRA|ICHRA|pension|529|health\s+savings\s+account|health\s+reimbursement\s+arrangement)\s+(?:contribution|benefit)\s*(?:of|:)?\s*"
    r"(?:USD|US\$|\$)\s*[\d,]+(?:\.\d+)?(?:\s*k)?"
    r"(?:\s*(?:[-–—]|to)\s*(?:USD|US\$|\$)?\s*[\d,]+(?:\.\d+)?(?:\s*k)?)?"
    r"|"
    r"(?:USD|US\$|\$)\s*[\d,]+(?:\.\d+)?(?:\s*k)?"
    ...
    r"(?:\s+(?:housing|living|relocation|meal|food|travel|wellness|phone|cell(?:phone)?|mobile|internet|commuter|parking|home\s+office|gym(?:\s+membership)?|fitness|child(?:[-\s])?care|dependent\s+care|health(?:care)?|WFH|work\s+from\s+home|remote\s+work|equipment|laptop|volunteer|tools|transit|education|moving|fertility|adoption|caregiver|parental|tech(?:nology)?|coworking|EAP|bike))*"
    r"\s+stipend\b"
    r"|"
    ...
    r"(?:\s*(?:[-–—]|to)\s*(?:USD|US\$|\$)?\s*[\d,]+(?:\.\d+)?(?:\s*k)?)?"
    r"(?:\s*(?:/\s*mo(?:nth)?s?|(?:per|a)\s+mo(?:nth)?s?|monthly))?"
    r"(?:\s+(?:housing|living|relocation|car|vehicle|auto|phone|cell|internet|meal|food|travel|commuter|parking|home\s+office|gym(?:\s+membership)?|fitness|child(?:[-\s])?care|dependent\s+care|health(?:care)?|clothing|uniform|WFH|remote|equipment|laptop|tech(?:nology)?|coworking|transit|mileage))*"
    r"\s+allowance\b"
    r"|"
    r"\b(?:(?:monthly|housing|living|relocation|car|vehicle|auto|phone|cell|internet|meal|food|travel|commuter|parking|clothing|uniform|WFH|remote|equipment|laptop|tech(?:nology)?|coworking|transit|mileage)\s+)*allowance\s*(?:of|:)?\s*"
    r"(?:USD|US\$|\$)\s*[\d,]+(?:\.\d+)?(?:\s*k)?"
    r"(?:\s*(?:[-–—]|to)\s*(?:USD|US\$|\$)?\s*[\d,]+(?:\.\d+)?(?:\s*k)?)?"
    r"|"
    r"(?:USD|US\$|\$)\s*[\d,]+(?:\.\d+)?(?:\s*k)?"
...
_ONSITE_WORKPLACE_RE = re.compile(
    ...
    r"|office\s+presence\s+"
    r"(?:two|three|four|five|\d+(?:\s*[-–]\s*\d+)?)\s+days?\s+(?:(?:a|per)\s+week|weekly)\b"
    r"|(?:two|three|four|five|\d+(?:\s*[-–]\s*\d+)?)\s+days?\s+(?:of\s+)?office\s+presence"
