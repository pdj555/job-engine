...
_FORTNIGHT_HOURS_RE = re.compile(
    ...
    r"(?:working\s+fortnights?|fortnights?|two\s+weeks|2\s+weeks|bi[-\s]?weekly|bi[-\s]?wkly|fn)\b"
    r"(?!\s+(?:meeting|standup|stand-up|sync|call|all-?hands))"
    r"|(?<![\d.])(\d{1,2}(?:\.\d+)?)\+?[\s-]*(?:hours?|hrs?|h)\.?\s+(?:fortnightly|bi[-\s]?weekly|bi[-\s]?wkly)\b"
    r"(?!\s+(?:meeting|standup|stand-up|sync|call|all-?hands))"
    r"|(?:hours?|hrs?|\bh)\s+(?:(?:per|a|each|every|/)\s+)(?:working\s+fortnights?|fortnights?|two\s+weeks|2\s+weeks|bi[-\s]?weekly|bi[-\s]?wkly|fn)\s*[:=\-–—]?\s*(\d{1,2}(?:\.\d+)?)\+?"
    ...
    r"(?:(?:per|a|each|every|/)\s+)(?:working\s+fortnights?|fortnights?|two\s+weeks|2\s+weeks|bi[-\s]?weekly|bi[-\s]?wkly|fn)\b"
