...
_NON_SALARY_MONEY_RE = re.compile(
    ...
    r"(?:USD|US\$|\$)\s*[\d,]+(?:\.\d+)?(?:\s*k)?"
    r"(?:\s*(?:[-–—]|to)\s*(?:USD|US\$|\$)?\s*[\d,]+(?:\.\d+)?(?:\s*k)?)?"
    r"\s+(?:(?:employee\s+)?(?:signing|sign-on|sign on|signon|relocation|annual|target|performance|retention|referral|spot|cash|year-end|year end|holiday|discretionary|quarterly|incentive|sales|monthly|stay|completion|anniversary|hiring|welcome|joining|new[-\s]hire)\s+bonus|(?:signing|sign-on|sign on|signon)\b)"
    r"|"
    r"\b(?:(?:employee\s+)?(?:signing|sign-on|sign on|signon|relocation|annual|target|performance|retention|referral|spot|cash|year-end|year end|holiday|discretionary|quarterly|incentive|sales|monthly|stay|completion|anniversary|hiring|welcome|joining|new[-\s]hire)\s+bonus|(?:signing|sign-on|sign on|signon))\s*(?:of|:)?\s*"
    r"(?:USD|US\$|\$)\s*[\d,]+(?:\.\d+)?(?:\s*k)?"
    r"(?:\s*(?:[-–—]|to)\s*(?:USD|US\$|\$)?\s*[\d,]+(?:\.\d+)?(?:\s*k)?)?"
    r"|"
    r"(?:USD|US\$|\$)\s*[\d,]+(?:\.\d+)?(?:\s*k)?"
