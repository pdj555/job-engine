...
_INDEX_TITLE_RE = re.compile(
    ...
    r"|\b(?:featured|latest|popular|hot|new|trending|recommended|matching|similar|suggested|related|other|browse|explore|view|discover) (?:positions|roles|listings|openings|opportunities)\b"
    r"|^life at\b"
...
_INDEX_PATH_RE = re.compile(
    ...
    r"|/(?:featured|latest|popular|hot|new|trending|recommended|matching|similar|suggested|related|other|browse|explore|view|discover)[-_]?(?:positions|roles|listings|openings|opportunities)/?$"
...
_GONE_LISTING_RE = re.compile(
    ...
    r"|(?:the|this)\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening)"
    r"\s+was\s+(?:withdrawn|cancelled|canceled|removed|taken\s+down|taken\s+offline|deactivated|unpublished|deleted|unposted)"
