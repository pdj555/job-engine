...
_HOURS_RE = re.compile(
    ...
    r"(?:/|\s*per[\s./]*(?:(?:the|each|every)\s+)?|\s+(?:a|each|every)\s+|\s+for(?:\s+(?:the|each|every))?\s+|\s+worked(?:\s+(?:per|/|a|each|every))?|\s+working(?:\s+(?:per|/|a|each|every))?|\s+scheduled(?:\s+(?:per|/|a|each|every))?|\s+work[\s-]*|\s+of\s+(?:the\s+)?(?:work(?:ing)?|scheduled)(?:\s+(?:for(?:\s+(?:the|each|every))?|a|per|each|every))?\s*|\s+of\s+(?:(?:the|each|every)\s+)?)?\s*"
