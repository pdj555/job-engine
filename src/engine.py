...
_HOURS_RE = re.compile(
    ...
    r"|(?:hours?|hrs?)\s*(?:(?:worked|working|scheduled)\s+)?(?:(?:p(?:er[\s.]*)?\.?\s*/?|/|an?\.?\s*/?|for(?:\s+the)?|each|every)\s*)?(?:weekly|wkly|weeks?|wk|w)\.?(?:\s+(?:of\s+(?:the\s+)?)?(?:work(?:ing)?|scheduled))?\s*(?:scheduled\s*)?[:=\-–—]?\s*(\d{1,2}(?:\.\d+)?)\+?"
