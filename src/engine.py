...
_GONE_LISTING_RE = re.compile(
    ...
    r"|we(?:'re| are)\s+no\s+longer\s+(?:recruiting|advertising)\s+this\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening)"
    r"|we(?:'ve|\s+have)?\s+stopped\s+(?:recruiting|advertising)\s+this\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening)"
    ...
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening)"
    r"|no\s+longer\s+(?:recruiting|advertising)\s+this\s+"
    r"(?:job(?:\s+posting)?|role|position|posting|vacancy|opportunity|requisition|req|listing|opening)"
...
_HOURS_RE = re.compile(
    ...
    r"|(?<![\d.])(\d{1,2}(?:\.\d+)?)[\s-]*(?:scheduled[\s-]+)?weekly[\s-]+(?:work(?:ing)?[\s-]+)?(?:hours?|hrs?)\b"
    r"(?!\s+(?:meeting|standup|stand-up|sync|call|all-?hands))"
