...
_ONSITE_WORKPLACE_RE = re.compile(
    ...
    r"|(?:must|should(?!n't)(?!\s+not)|(?<!not )(?<!n't )ha(?:ve|s|d)\s+to)\s+(?:clock|check|sign)\s+in\s+at\s+"
    r"(?:the\s+|our\s+|an\s+)?(?:\S+\s+){0,4}(?<!home )(?<!home-)(?:offices?|campus|lab(?:oratory)?|headquarters|hq)\b"
    r"|(?<!not )(?:required|expected|expects?|need(?:s|ed)?|asked|requested|requests?|invited|encouraged|preferred|prefer|supposed|ought|instructed|directed|mandated|obligated|compelled|ordered|told)\s+to\s+(?:clock|check|sign)\s+in\s+at\s+"
    r"(?:the\s+|our\s+|an\s+)?(?:\S+\s+){0,4}(?<!home )(?<!home-)(?:offices?|campus|lab(?:oratory)?|headquarters|hq)\b"
