...
_NON_SALARY_MONEY_RE = re.compile(
    ...
    r"|"
    r"\b(?:mileage|clothing|equipment|laptop\s+stand|laptop\s+sleeve|laptop\s+bag|laptop\s+case|laptop\s+backpack|laptop|macbook|chromebook|thinkpad|surface\s+pro|surface\s+book|moving|uniform|bike|coworking|adoption|fertility|parental|WFH|tools|EAP|sabbatical|PTO|bereavement|options|copay|deductible|coinsurance|out(?:-|\s+)of(?:-|\s+)pocket|subsidy|perk|voucher|discount|cellphone|wifi|wi-fi|broadband|gear|headset|headphones?|earbuds?|earphones?|airpods|external\s+monitor|second\s+monitor|webcam\s+stand|phone\s+stand|tablet\s+stand|ipad\s+stand|kindle\s+stand|keyboard\s+tray|keyboard\s+stand|monitor\s+arm|monitor\s+stand|monitor\s+mount|monitor\s+riser|monitor|keyboard|ergonomic\s+mouse|wireless\s+mouse|mouse|mousepad|mouse\s+pad|trackpad|dock|docking\s+station|thunderbolt\s+dock|thunderbolt\s+\d\s+cable|firewire\s+cable|fire\s+wire\s+cable|optical\s+cable|toslink\s+cable|audio\s+cable|aux\s+cable|thunderbolt\s+cable|thunderbolt\s+adapter|hdmi\s+dongle|usb[-\s]?c\s+dongle|display(?:\s+|-)?port\s+dongle|thunderbolt\s+dongle|dp\s+dongle|ethernet\s+dongle|dongle|usb[-\s]?c\s+hub|thunderbolt\s+hub|display\s+hub|hdmi\s+hub|usb\s+hub|usb[-\s]?c\s+cable|usb\s+cable|hdmi\s+cable|charging\s+cable|charge\s+cable|lightning\s+cable|vga\s+(?:cable|adapter)|dvi\s+(?:cable|adapter)|display\s+cable|video\s+cable|power\s+cable|extension\s+cable|usb[-\s]?c\s+adapter|usb\s+adapter|hdmi\s+adapter|display(?:\s+|-)?port\s+(?:hub|cable|adapter|switch)|dp\s+(?:hub|cable|adapter|switch)|mini(?:\s+|-)?displayport\s+cable|mini\s+dp\s+cable|minidp\s+cable|sata\s+(?:hub|cable|adapter)|flash\s+drive|thumb\s+drive|usb\s+drive|micro[\s-]?sd\s+card|sd\s+card|memory\s+card|external\s+ssd|external\s+hdd|hard\s+drive|hard\s+disk|yubikey|security\s+key|ethernet\s+(?:hub|cable|adapter|switch)|hdmi\s+(?:switch(?:er)?|splitter|extender|repeater)|usb[-\s]?c\s+switch|usb\s+switch|kvm\s+switch|wireless\s+charger|usb[-\s]?c\s+charger|magsafe\s+charger|wall\s+charger|car\s+charger|charger|power\s+bank|powerbank|power\s+brick|chair|standing\s+desk|webcam|microphones?|mic|printer|scanner|copier|ergonomic\s+chair|hotspot|router|modem|smartphone|iphone|ipad|tablet|kindle|data\s+plan|cellular\s+plan|mobile\s+plan|phone\s+plan|cell\s+plan|sim\s+card)\s*(?:of|:)?\s*"
    r"(?:USD|US\$|\$)\s*[\d,]+(?:\.\d+)?(?:\s*k)?"
    r"(?:\s*(?:[-–—]|to)\s*(?:USD|US\$|\$)?\s*[\d,]+(?:\.\d+)?(?:\s*k)?)?"
    r"|"
