...
_HOURS_RE = re.compile(
    ...
    r"|(?:hours?|hrs?)\s*(?:(?:worked|working|scheduled)\s+)?(?:(?:p(?:er[\s.]*)?\.?|/|an?\.?|for(?:\s+the)?)\s*)?(?:wk|weeks?|weekly)\s*(?:scheduled\s*)?[:=\-–—]?\s*(\d{1,2}(?:\.\d+)?)\+?"
    ...
    r"(?!\s+(?:meeting|standup|stand-up|sync|call|all-?hands))"
    r"|(?:hours?|hrs?)\s*(?:worked\s*)?[:=\-–—]\s*(\d{1,2}(?:\.\d+)?)\+?\s*(?:p(?:er[\s.]*)?\.?|/|an?\.?\s*)(?:wk|weeks?|weekly)\b"
    r"|work[\s-]*weeks?\s*[:=\-–—]\s*(\d{1,2}(?:\.\d+)?)\+?\s*(?:hours?|hrs?)?\b"
    r"|(?:hours?|hrs?)\s+of\s+(?:the\s+)?(?:work(?:ing)?|scheduled)\s*(?:(?:p(?:er[\s.]*)?\.?|/|an?\.?)\s*)?(?:wk|weeks?|weekly)\s*[:=\-–—]?\s*(\d{1,2}(?:\.\d+)?)\+?"
    r"|(?<![\d.])(\d{1,2}(?:\.\d+)?)\+?[\s-]*hpw\b"
